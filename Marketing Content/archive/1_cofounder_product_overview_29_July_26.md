...
### 25. Monthly Judgment Review ("Open Loop")
...
### 28. Access tiers & monetization

Free users get the full Council experience and the Decision Brief. A paid Mirror subscription (monthly or annual) unlocks the longitudinal layer — Fingerprint, Calibration, Contradictions, Session Reliability Index, Monthly Judgment Review, Peer Benchmark, full Decision Graph — with a couple of the deeper features (like cross-user benchmarking) only unlocking once someone has enough session history. This is now a real self-serve payment flow rather than a manual one: Mirror subscriptions run end-to-end through Razorpay — checkout, recurring billing, and self-serve cancellation — with manual grants and one-off unlock codes still available alongside it for outreach and comps. A separate "Advisory" tier — a small, capped, founder-led cohort granted manually rather than self-serve — gets everything immediately with no session-count wait, plus a few things that stay deliberately offline: a quarterly written judgment memo and a founder call.
