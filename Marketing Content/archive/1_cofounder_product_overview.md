...
5. **A built-in questioner asks two or three sharp follow-ups on every decision** — even a redirected one gets a question, just a sharper, narrower one aimed at resolving whatever tripped the flag. The standard version isn't generic journaling prompts; it's built around what's emotionally at stake, what's still ambiguous, and what your own history suggests you tend to skip over.
6. **A synthesis pulls it together** — a single headline verdict and a named central tension, visually distinct from the six individual takes.
7. **You rate your confidence, and later come back and log what actually happened.** This is the hinge the whole long-term product turns on — without it, Quorum is a smart chatbot. With it, Quorum starts learning your patterns.
8. **Over many decisions**, Quorum builds a private profile of your blind spots, your calibration (are you usually too confident or too cautious?), contradictions between what you say and what you do, and a visual map connecting today's decision to past ones that rhyme with it.
...
### 7. Council Weighting — shown, not hidden
...
### 8. Bias scoring — 15 named biases, tracked two ways
...
### 9. The real differentiator — bias *triggers*, not just bias labels
...
By comparing your stated confidence at decision-time against what actually happened, Quorum classifies you — per decision dimension — as overconfident, underconfident, or well-calibrated: the same skill professional forecasters train explicitly.
...
### 16. The Decision Brief

A polished, exportable PDF of the full session — every advisor's take, the follow-up Q&A, and an executive-style synthesis — available free to everyone. It works as both a personal artifact and a natural share/referral surface.

### 17. Nudges & re-engagement

A quiet background system with a few distinct jobs: a rotating daily prompt for people who haven't logged a decision recently, an outcome-logging reminder that waits about a week before ever asking (so there's actually been time for something to happen) and stops asking after two months, a gentle post-session nudge toward the decision graph that's throttled so you're never prompted more than once every few days regardless of which trigger fired, and a short escalating sequence for genuinely lapsed users. Nothing here stacks — a user never gets hit with two competing prompts for the same moment.

### 18. Access tiers & monetization

Free users get the full Council experience and the Decision Brief. A paid Mirror subscription (monthly or annual) unlocks the longitudinal layer — Fingerprint, Calibration, Contradictions, full Decision Graph — with a couple of the deeper features (like cross-user benchmarking) only unlocking once someone has enough session history. A separate "Advisory" tier — a small, capped, founder-led cohort granted manually rather than self-serve — gets everything immediately with no session-count wait, plus a few things that stay deliberately offline: a quarterly written judgment memo and a founder call.
...
**Cost is managed by routing, not by cutting corners.** The advisor reasoning and anything analytically sensitive runs on a premium model; lower-stakes, higher-volume tasks run on a cheaper one. That's a unit-economics decision, not a quality one.

**A user can start completely anonymously** and still accumulate history under a private identity — if they later give an email or sign up, that history follows them forward. The product proves itself before it asks for anything.
