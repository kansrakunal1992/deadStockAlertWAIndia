# Quorum — Product & Architecture Overview (v2)
...
Three named plans now (the "Locked v1" pricing pass): **Free**, **Elite**, and **Private**. Free users get the full Council experience and the Decision Brief, no card required. Elite (₹2,999/mo or ₹29,999/yr) unlocks the longitudinal Mirror layer — Fingerprint, Calibration, Contradictions, Session Reliability Index, Monthly Judgment Review, Peer Benchmark, full Decision Graph — with a couple of the deeper features only unlocking once someone has enough session history. Elite runs end-to-end through Razorpay — checkout, recurring billing, and self-serve cancellation — with manual grants and one-off unlock codes still available alongside it for outreach and comps. **Private** is new: a self-hosted deployment (custom pricing, starting around ₹9,999/user/month) for buyers who want their data and the AI model itself kept off shared infrastructure — sold, not self-served, since it needs a minimum-seats conversation. Buyers choose between two model options at sale time, disclosed plainly rather than defaulted quietly: a stronger option and a lower-cost, lower-reasoning-quality option. **This is no longer just a pricing line on paper** — a real one-click deployment system now exists behind it: a buyer's chosen cloud account (AWS, GCP, or Azure) gets its own dedicated instance that provisions itself and keeps itself updated automatically, with no ongoing hands-on work from us once it's live. What's still genuinely unproven: GPU sizing hasn't been validated against real usage yet, and cloud GPU quota approvals can take a few days — both worth raising early in a sales conversation rather than discovering them mid-deal.

**A capped early-access offer, "Founding Elite," now sits alongside standard Elite pricing** — the same full Elite feature set, at ₹999/mo or ₹9,999/yr instead of the regular ₹2,999/₹29,999, for the first 20 people who take it. It's not a fourth plan or a different product — just a discounted entry price for an early cohort, and once someone's in, they keep that status even if their subscription later lapses (the 20 seats don't reopen). It shows up as a specific, warmer call-to-action ("Become a Founding Member") wherever the product would otherwise nudge a Free user toward Elite, for as long as the cohort has room.
