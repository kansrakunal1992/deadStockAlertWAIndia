...
async function updateBatchQuantity(batchId, quantityChange) {
    ...
    try {
        ...
        if (!batchResult || !batchResult.fields) {
            throw new Error('Batch record not found');
        }
        
        const currentQuantity = batchResult.fields.Quantity || 0;
        const newQuantity = currentQuantity + quantityChange; // quantityChange is negative for sales
        ...
        console.log(`[${context}] Batch quantity updated successfully: ${currentQuantity} -> ${newQuantity}`);
        return { success: true };
    } catch (error) {
        logError(context, error);
        return {
...
// Get sales records for a shop from the Sales table
