...
const context = `Create Sales Record ${salesData.shopId} - ${salesData.product}`;
try {
console.log(`[${context}] Creating sales record for ${salesData.quantity} units`);
const createData = {
fields: {
ShopID: salesData.shopId,
Product: salesData.product,
Quantity: Math.abs(salesData.quantity), // Store as positive number
SaleDate: salesData.saleDate || new Date().toISOString(),
...
async function getShopSalesRecords(shopId, days = 1) {
...
// Update batch quantity when a sale is made
async function updateBatchQuantity(batchId, quantitySold) {
const context = `Update Batch Quantity ${batchId}`;
try {
console.log(`[${context}] Updating batch ${batchId} quantity by ${quantitySold}`);

// First, get the current batch record
const batchResult = await airtableBatchRequest({
method: 'get',
url: `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${BATCH_TABLE_NAME}/${batchId}`
}, `${context} - Get Batch`);
...
const currentQuantity = batchResult.records[0].fields.Quantity || 0;
const newQuantity = Math.max(0, currentQuantity - Math.abs(quantitySold));
