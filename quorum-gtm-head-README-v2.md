...
**Known, stated-plainly gap:** `/kunal_elite` bookings happen entirely
inside an embedded Google Calendar widget — there is no equivalent to
`decision_session_payments` for the free session. `card_visit_log` (did
they land on the page) is the best signal available until a Google
Calendar API integration exists (would need your OAuth credentials — not
built, flagged for later if you want it).
...
**Lifecycle nurture agent — deliberately NOT built this pass.** Your main
app already runs five sophisticated nudge crons (daily-nudge,
validation-nudge, reanalyze-email, avoidance-detect, mirror-insight-email),
sharing a throttle gate so no user gets double-nudged. `mirror-insight-email`
already targets almost exactly the segment this was meant for (engaged,
not-yet-paid users). Building GTM Head's own nurture without deliberately
designed non-overlap risks duplicate/spammy emails to real users — worth a
short specific proposal before writing that code, not a default extension
of everything else in this pass.
