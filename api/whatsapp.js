...
// Improved greeting detection function
...
if (/[\u0900-\u097F]/.test(text)) return 'mr'; // Marathi script
...
if (/[\u0C80-\u0CFF]/.test(text)) return 'kn'; // Kannada script
if (/[\u0A80-\u0AFF]/.test(text)) return 'gu'; // Gujarati script
if (/[\u0D00-\u0D7F]/.test(text)) return 'mr'; // Marathi script
...
// CRITICAL FIX: Proper action handling with negative quantities for sales
if (isSold) {
action = 'sold';
finalQuantity = -Math.abs(quantity); // Always negative for sales
} else if (isPurchase) {
...
// CRITICAL FIX: Add Hindi to English product mapping
const hindiToEnglish = {
'चीनी': 'sugar', 'cheeni': 'sugar',
'दूध': 'milk', 'doodh': 'milk',
'आटा': 'flour', 'aata': 'flour',
'नमक': 'salt', 'namak': 'salt',
'आलू': 'potato', 'aaloo': 'potato',
...
const lowerProductName = productName.toLowerCase();
if (hindiToEnglish[lowerProductName]) {
console.log(`[${requestId}] Translated product: "${productName}" → "${hindiToEnglish[lowerProductName]}"`);
// Cache the result
productTranslationCache.set(cacheKey, {
translation: hindiToEnglish[lowerProductName],
timestamp: Date.now()
});
return hindiToEnglish[lowerProductName];
}
