...
async function parseMultipleUpdates(transcript) {
    ...
    console.log(`[Rule-based Parsing] Attempting to parse: "${transcript}"`);
...
function parseSingleUpdate(transcript) {
    // Extract units first - be more specific about unit detection
    let unit = '';
    let unitMultiplier = 1;
    
    // More precise unit extraction - look for specific unit words
    const unitPatterns = [
        /(\d+)\s*(kg|किलो|kilo|kilogram|kilograms)/gi,
        /(\d+)\s*(g|gram|grams|ग्राम)/gi,
        /(\d+)\s*(packets|पैकेट)/gi,
        /(\d+)\s*(boxes|बॉक्स)/gi,
        /(\d+)\s*(pieces|पीस)/gi,
        /(\d+)\s*(liters|लीटर|litre|litres)/gi,
        /(\d+)\s*(ml|milliliter|milliliters|millilitre|millilitres)/gi
    ];
    ...
    for (const pattern of unitPatterns) {
        ...
        if (match) {
            ...
            if (unitWord === 'g' || unitWord === 'gram' || unitWord === 'grams' || unitWord === 'ग्राम') {
                unitMultiplier = 0.001; // Convert grams to kg
            } else {
                unitMultiplier = 1; // Default multiplier
            }
            break;
        }
    }
...
function extractProduct(transcript) {
    // Don't remove common product suffixes like "biscuits", "chips", etc.
...
function isExpiryDateUpdate(message) {
    ...
    // More specific pattern for expiry date updates
    const expiryPattern = /([a-zA-Z\s]+):\s*(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}|\d{1,2}\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4})/i;
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (Body && (NumMedia === '0' || NumMedia === 0 || !NumMedia)) {
            ...
            // Check for common greetings with expanded language support
            const greetings = {
                'en': ['hello', 'hi', 'hey'],
            ...
            // Try to parse as inventory update first
            ...
            if (updates.length > 0) {
                ...
                // Check for batch selection or expiry date updates only if in appropriate state
                if (conversationState && conversationState.state === 'awaiting_batch_selection') {
