...
### mirror_access ← updated Sprint 19 migration; access_type retired 'lifetime' June 13, 2026
```
id               uuid PK
...
| Decision Brief | Pay-per-brief via `BRIEF_ACCESS_TOKEN` (unchanged) | **Not yet wired** — deferred, see §7 |
...
- **Decision Brief advisory bypass is deferred.** `BriefCTA` lives in `app/record/[id]/page.tsx`, a different page from `/mirror`, and `/api/brief-access` is currently a stateless token check by design ("no DB, no accounts needed"). Making it advisory-aware requires new auth/tier plumbing into the record page — a separable follow-up, not done in this sprint.
