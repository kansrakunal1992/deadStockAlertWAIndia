...
**Private tier infra note:** the self-hosted Qwen/Mistral endpoints do not exist yet — separate infra track (GPU provisioning, deploy tooling). The Private branch in `ai-client.ts` is fully wired but fails loudly with a clear error if its endpoint env vars are unset, rather than silently substituting a different tier's model. A caught error from this branch means the infra isn't live, not that Private tier is broken.
