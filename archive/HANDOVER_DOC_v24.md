# QUORUM — Handover Document v24
### Date: May 2026 | Status: Sprint 21 complete (Style calibration component · Persona reorder by user style · Calibration persistence fix)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Sprint 23b · Status: 🔲 Not started
TTS — Read Aloud on Council Synthesis card.
Establishes all TTS infrastructure (API route, hook, context) that Sprint 23c reuses without modification.

Architecture decision — REST not WebSocket
Soniox TTS offers two modes: real-time WebSocket (text streams in, audio streams back) and REST generation (full text in, audio back). Use REST for Quorum TTS because:

Synthesis and persona text is already fully available when user clicks "Read aloud" — no benefit to streaming input
REST is stateless: no session Map, no SSE, no Railway persistence dependency
REST is simpler to debug, retry, and test


Soniox TTS REST API — what to verify before implementing
Before writing any code, open: https://soniox.com/docs/tts/rest/rest-generation
Confirm:

Exact endpoint URL — expected pattern (unconfirmed, must verify): POST https://api.soniox.com/v1/synthesize
...
Response — expected: binary audio stream (Content-Type: audio/mpeg or audio/wav)
Authentication — expected: Authorization: Bearer SONIOX_API_KEY header (same key as STT)

Voice model selection: Open https://soniox.com/docs/tts/models and pick an English voice. Look for a neutral, authoritative male or female English voice — ideally en-GB if available (sounds more boardroom-appropriate than en-US). Note the exact model ID string and hardcode it in the route.
Audio format: Request mp3 — universally playable in browser Audio element without any decoding. If Soniox doesn't support mp3, use wav as fallback.

Files to create
app/api/voice/tts/route.ts — NEW
Server-side proxy. Accepts { text: string } from client, calls Soniox TTS REST, streams audio back.
export const runtime = 'nodejs'

POST body: { text: string }

Steps:
1. Check SONIOX_API_KEY — return 503 STT_NOT_CONFIGURED if missing
2. Parse body, validate text is non-empty string, max 8000 chars (guard)
3. Strip markdown before sending to Soniox:
   - Remove **bold**, *italic*, _underline_, # headings, bullet markers (- •)
   - Simple regex replacements — do NOT use a markdown library
   - Regex: text.replace(/\*\*(.+?)\*\*/g, '$1').replace(/\*(.+?)\*/g, '$1').replace(/^#+\s*/gm, '').replace(/^[-•]\s*/gm, '')
4. POST to Soniox TTS REST endpoint with API key in Authorization header
5. On Soniox error: log error_type (stable), return 502 with typed error code
6. On success: pipe Soniox audio response body directly back to client
   - Set Content-Type to match Soniox response (audio/mpeg or audio/wav)
   - Set Cache-Control: no-store
   - Return new Response(sonioxRes.body, { headers })

Error codes to return (same pattern as voice/transcribe):
  TTS_NOT_CONFIGURED  — API key missing
  TTS_QUOTA_EXCEEDED  — 429 from Soniox
  TTS_PROVIDER_DOWN   — 5xx from Soniox
  TTS_FAILED          — other non-ok response
  INPUT_TOO_LONG      — text > 8000 chars
hooks/useSonioxTTS.ts — NEW
Client-side TTS hook. Calls /api/voice/tts, plays audio via browser Audio element.
Interface:
  speak(text: string, speakerId: string): void
  stop(): void
  isSpeaking: boolean
  isLoading: boolean         — true while fetching audio from server
  activeSpeakerId: string | null

State machine:
  idle → loading → speaking → idle
       ↘ error (show nothing, fail silently — TTS is enhancement not core)

Implementation notes:
  - Use useRef for the Audio element: audioRef = useRef<HTMLAudioElement | null>(null)
  - On speak(): if audioRef.current playing, stop it first
  - fetch('/api/voice/tts', { method: 'POST', body: JSON.stringify({ text }) })
  - On success: res.blob() → URL.createObjectURL(blob) → new Audio(url) → audio.play()
  - Store object URL in ref, call URL.revokeObjectURL on stop/next speak (memory leak guard)
  - audio.onended: setIsSpeaking(false), setActiveSpeakerId(null)
  - On fetch error: silent fail — setIsLoading(false), no error state shown to user
  - Cleanup on unmount: stop audio, revoke object URL

Singleton enforcement (done via TTSContext, not this hook — see below):
  This hook is NOT a singleton itself. TTSContext wraps it as a singleton.
  Do NOT call useSonioxTTS directly in components — always use useTTSContext().
context/TTSContext.tsx — NEW
React context that wraps useSonioxTTS as a singleton. Ensures only one card speaks at a time across PersonaPanel (×6) and SynthesisCard.
Interface exported:
  TTSContextValue {
    speak:           (text: string, speakerId: string) => void
    stop:            () => void
...
Implementation:
  - TTSProvider wraps the app (mount in app/council/[sessionId]/page.tsx or layout)
  - TTSContext created with createContext<TTSContextValue>(...)
  - TTSProvider calls useSonioxTTS internally, exposes via context value
  - Export useTTSContext() convenience hook (throws if used outside provider)

Mount point:
  Council page renders SynthesisCard + 6 PersonaPanel components.
  Wrap them in <TTSProvider> in app/council/[sessionId]/page.tsx.
  DO NOT mount at app layout level — TTS is council-only.

speakerId convention:
  - SynthesisCard: speakerId = 'synthesis'
  - PersonaPanel: speakerId = persona.key (e.g. 'risk_architect', 'pattern_analyst' etc.)
  Each card checks: activeSpeakerId === its own speakerId → to decide which icon state to show

Files to modify
components/SynthesisCard.tsx — EDIT
Add "Read aloud" button in the Mirror nudge row (the state === 'done' section at line ~473).
Placement: In the existing mirror nudge div (justifyContent: space-between),
add the TTS button on the LEFT, before the mirror nudge text.

The row currently has:
  [This decision has been added to your Mirror profile.]  [View Mirror →]

After edit:
  [▶ Read aloud / ■ Stop]  [This decision has been added...]  [View Mirror →]

Button behaviour:
  - Import useTTSContext
  - const { speak, stop, isSpeaking, isLoading, activeSpeakerId } = useTTSContext()
  - const isThisSpeaking = activeSpeakerId === 'synthesis'
  - Text to speak: synthesis state value (strip markdown before passing to speak())
    Stripping is done server-side in route, but pass clean text here too for safety
  - While isLoading && isThisSpeaking: show Spinner (same as VoiceInput)
  - While isSpeaking && isThisSpeaking: show ■ Stop icon + "Stop" label
  - Default: show ▶ icon + "Read aloud" label
  - Only show button when state === 'done' && synthesis (same condition as mirror nudge)
  - Button style: match existing "View Mirror →" link weight — subtle, not dominant
    fontSize: 12, color: var(--text-3), border: 1px solid var(--border-dim)
    On active: color: var(--gold), border: var(--gold-dim)

Sprint 23b test checklist
#TestExpectedB1Click Read aloud on synthesisBrief loading (isLoading) → audio begins ~1-2sB2Click Stop mid-audioAudio stops immediatelyB3Click Read aloud again after stopNew fetch, plays from beginningB4Navigate away while playingunmount cleanup stops audio, revokes URLB5Network tabSONIOX_API_KEY not in any client requestB6Synthesis contains bold markersStripped — Soniox doesn't read "asterisk asterisk"B7Soniox TTS 429Button returns to idle silently (TTS fail is non-fatal)B8Light modeButton renders correctly (CSS vars only)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Sprint 23c · Status: 🔲 Not started
TTS — Read Aloud on all 6 persona cards. Reuses Sprint 23b infrastructure entirely.
Prerequisite: Sprint 23b must be complete and deployed before starting 23c.

No new files
23c is purely edits to components/PersonaPanel.tsx. All TTS infrastructure
(route, hook, context) is already live from 23b.

Files to modify
components/PersonaPanel.tsx — EDIT
Add speaker icon button in the persona card header, in the existing right-side button group.
Exact location in file:
The header at line ~249 has a right-side <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}> that already contains the pushback button. Add the TTS speaker button BEFORE the pushback button in this group.
Imports to add:
...
Button placement rules:
  - Show ONLY when panelState === 'done' (text is fully loaded, safe to read)
  - Sits alongside the existing pushback button in the header right group
  - Pushback button already has condition: panelState === 'done' && !isPushingBack && ...
    Add TTS button with the same condition wrapper

Button behaviour:
  - isThisSpeaking && isLoading: Spinner (13px), no label — header space is tight
  - isThisSpeaking && isSpeaking: ■ Stop icon, no label
  - default: speaker/volume icon, no label — icon-only in header to avoid crowding
  - On click when not speaking: speak(content, persona.key)
    where `content` is the current persona text (the streaming content string stored in component state)
    Find the correct state variable name — it is `content` in panelState === 'done'
    Check the existing state variable that holds the completed persona text and use that
  - On click when isThisSpeaking: stop()

Speaker icon SVG (use this exactly — fits header button size):
...
Button style (matches persona header aesthetic — light on dark accent bg):
  ...
  On hover: background rgba(255,255,255,0.18), color white

Active state (isThisSpeaking):
  background: rgba(201,168,76,0.20)
  border: 1px solid var(--gold-dim)
  color: var(--gold)

One-at-a-time enforcement:
  Already handled by TTSContext singleton — clicking Persona B while Persona A
  is speaking automatically stops A and starts B. No per-card coordination needed.
  The activeSpeakerId check in each card drives the button state automatically.

Singleton enforcement — how it works end to end
TTSProvider (in council page)
  └── useSonioxTTS (one instance, one Audio element)
        ├── SynthesisCard → useTTSContext() → speak('synthesis')
        ├── PersonaPanel[risk_architect] → useTTSContext() → speak('risk_architect')
        ├── PersonaPanel[pattern_analyst] → useTTSContext() → speak('pattern_analyst')
        └── ... (all 6 personas)

When Persona A is speaking and user clicks Persona B:
  speak('persona_b_key') called
  → useSonioxTTS.speak() sees audioRef.current is playing
  → stops current audio, revokes URL
  → sets activeSpeakerId to 'persona_b_key'
  → fetches new audio for Persona B
  → Persona A button: activeSpeakerId !== 'risk_architect' → shows idle icon
  → Persona B button: activeSpeakerId === 'persona_b_key' → shows loading → playing

Sprint 23c test checklist
#TestExpectedC1Click speaker on Persona 1Audio beginsC2Click speaker on Persona 2 while P1 playingP1 stops; P2 begins; P1 button returns to idleC3Click stop on active personaAudio stops immediatelyC4Click synthesis Read aloud while persona playingPersona stops; synthesis beginsC5All 6 personas loaded → click each speakerEach plays correctly, no overlapC6Persona still streaming (not done)Speaker button hidden/disabledC7After pushback re-run (examiner update)Speaker plays updated persona text, not originalC8Light modeButton styles correct (persona header is accentColor bg regardless of theme)

Environment — no new vars needed for 23b or 23c
SONIOX_API_KEY already added in Sprint 23a. Same key covers both STT and TTS.

Known risk — Soniox TTS REST endpoint URL
The exact REST endpoint URL was not confirmed against Soniox docs at time of writing.
Before implementing app/api/voice/tts/route.ts:

Go to https://soniox.com/docs/tts/rest/rest-generation
Copy the exact POST URL from their code example
Confirm request body field names (model, text, audio_format)
Go to https://soniox.com/docs/tts/models and pick the voice model ID

STT precedent: raw docs showed URL was wss://stt-rt.soniox.com/transcribe-websocket,
not the guessed wss://api.soniox.com/v1/speech-to-text — verify TTS similarly.
