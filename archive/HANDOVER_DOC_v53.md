# QUORUM — Handover Document v53

> **Note on the entries directly below (July 28 → July 20, 2026):** reconstructed the same way as the two blocks further down (diffing the uploaded codebase against the last documented baseline, currently July 17) — again with **no `.git` directory present** in either of two deliveries this pass (a full codebase zip, then a small follow-up re-upload of 3 changed files a few hours later), so dates/times below are `mtime`-based, not commit-log-based. The two deliveries themselves bracket a clean seam: the second upload's `diff -rq` against the first touched exactly 3 files (`lib/ai-client.ts`, `lib/personas.ts`, `app/api/persona/route.ts`) — see the July 28 (17:19 UTC) entry — so that entry's timestamp is unusually precise, filesystem-confirmed rather than inferred from a wider batch. Same caveat as those other two blocks applies here: **no new KDD/TD numbers assigned in this pass**; items flagged "KDD-worthy" below are for you to number. Two of these entries (July 20, and July 21–25) cover ground that the Technical Spec document already documents in full — its own author evidently worked from the same or a later snapshot of this codebase and wrote §2.19–2.22, §6.3, and §9.2–9.5 from scratch, all marked "not previously documented" there. Rather than re-deriving that detail a second time, the entries below stay in this doc's usual narrative/KDD-flagging register and point at the relevant TSD section for the full mechanism.
>
> **Worth naming directly: two separate, real pricing/business-model facts surfaced this pass that this document (and, in one case, the Technical Spec) had gone stale on.** (1) Mirror/Elite pricing is **₹2,999/month · ₹29,999/year** — the Technical Spec's Payments section (§9, pre-this-pass) still listed ₹3,999/₹39,999; corrected there this pass, flagged here so it doesn't get silently reintroduced from an old copy of that section. (2) The standalone "Advisory" tier has been **retired as a distinct product tier (Phase 6)** — its feature advantages folded into Elite; Advisory now survives only as a provenance marker for how an account was granted access. See the July 27 entry below and TSD §9.5.

### Date: July 28, 2026 (≈17:19 UTC) | Status: Mistral-Family Prompt Quality Extension — 3 files touched (`lib/ai-client.ts`, `lib/personas.ts`, `app/api/persona/route.ts`), delivered as a standalone follow-up re-upload a few hours after the rest of this pass's batch, cleanly isolated from everything else below (confirmed via `diff -rq` against the prior delivery — these were the only 3 files that changed). A side-by-side comparison run (same decision, same context, same six-advisor council, no examiner questions answered) across DeepSeek V4 Pro and Mistral Small surfaced a real quality gap on Mistral specifically: it invented stakeholders not present in the supplied decision/context (a co-founder, an angel investor, a first customer, a seed round), and tended to summarise the six advisors rather than synthesise across them, where DeepSeek reliably found "the question beneath the question" and reached for precise structural language instead. This matters now specifically because Free tier and part of Elite route persona/synthesis calls to Mistral under the tiered-routing system shipped earlier this same pass (see July 27 below) — a quality gap that was previously academic (nothing routed to Mistral in production) is now a live product-quality question. **`lib/ai-client.ts`** — new `getModelFamily(requested?)`, a side-channel peek that re-runs `resolveProvider()`'s exact resolution logic and returns just the resulting family, so a caller can know which model family a call will hit *before* finishing that call's `systemPrompt` (the actual resolution otherwise only happens inside `createStream`/`createCompletion`, after the prompt is already built). **`lib/personas.ts`** — three new exported prompt-extension constants (`MISTRAL_EVIDENCE_DISCIPLINE`, `MISTRAL_SYNTHESIS_PUSH`, `MISTRAL_PERSONA_PUSH`), appended only when the model family resolves to Mistral. **`app/api/persona/route.ts`** — new Layer 6 in the systemPrompt assembly, conditional on `getModelFamily()`'s result; Claude and DeepSeek calls are byte-identical to before this change. Deliberately scoped to the six personas + synthesis only — Examiner and structured-extraction calls (ontology tagger, bias scorer) share the same exposure in principle but weren't covered by the reference transcripts, and extending the fix to them is logged as a recommended follow-on rather than bundled in. Full mechanism in TSD §2.22.

### Date: July 28, 2026 (≈06:39–07:18 UTC) | Status: Session Sharing ("Share my decision") — new feature, 6 files/1 migration (`supabase/add_record_sharing.sql`; `app/api/record/[id]/share/route.ts` new; `app/share/[token]/page.tsx` new; `lib/synthesis-highlights.ts` new; `components/ShareRecordButton.tsx` new; `app/page.tsx` touched, presumably to surface the button on the homepage session list). An opt-in, revocable public read-only link per session — off by default, flips on only via an explicit owner click, never on mount. Public page is deliberately narrow (decision text, context, latest verdict only — no persona debate, no bias/Mirror data, none of it even fetched), gated on `is_shared = true` so a revoked link can't be reached even if the token leaks. Share composer builds a WhatsApp/LinkedIn/Reddit-ready message from the *latest* synthesis message in the session (a challenged/pushback exchange can produce more than one), and the public page sets Open Graph metadata directly on the URL for a real link-preview card — the only lever available for that, since none of those platforms read anything from `wa.me`/Reddit-style query params. Re-enabling a previously-revoked share reuses the same token/URL rather than minting a new one, so a link already sent out doesn't quietly go dead. **New shared dependency, not a fourth copy:** `lib/synthesis-highlights.ts` is a fourth independent parser of the same `<verdict>`/`<conditions>`/`<key_question>`/`<action_plan>` tags already duplicated across `SynthesisCard.tsx`, `RecordExport.tsx`, and the record page's own `parseVerdictTension()` — this document and the TSD have flagged that duplication repeatedly (§2.14/§2.20); this file was written specifically so the share route and share page have one dependency between them rather than adding a fourth copy of the tag-stripping logic. **Flagging as KDD-worthy:** this new parser is not covered by the §2.20 tag-wiring guardrail tests (`tests/tag-wiring-guardrail.test.ts` only enumerates the five pre-existing sinks) — if the share feature's tag set ever grows to include a tag the other four sinks handle with a truncation-guard fallback, nothing currently catches a missing guard here the way it would for the other four. Full mechanism in TSD §2.21.

### Date: July 28, 2026 (≈01:29–02:31 UTC) | Status: Google Sign-In UI wiring + Decision Rules / Mirror Graph touch-ups — continuation of the July 27 Sprint 12 work below, same day's second half. `lib/google-auth.ts` (new) — extracts the Google `signInWithOAuth` redirect call out of `AuthPanel.tsx` into one shared `signInWithGoogle()` function, specifically so `EmailCaptureCard.tsx` and `app/mirror/page.tsx`'s `AuthGate` (both touched the same window) can offer "Continue with Google" without re-declaring the redirect logic a third time. `AuthPanel.tsx` itself updated ~30 minutes later to consume the new shared function and render the `provider_mismatch` messaging inline. Also touched in roughly the same window, apparently unrelated to the auth work specifically: `app/settings/privacy/page.tsx`, `components/RecordTour.tsx`, `components/DecisionRules.tsx`, `app/api/mirror/rules/route.ts`, `app/api/mirror/graph/route.ts`, `components/ContradictionDetector.tsx` — the latter three's actual content changes are the Phase 6 Advisory-tier-retirement edits (removing the session-count-bypass branch and the now-dead `ContradictionSummaryView` sub-component), so these are best read as part of the July 27 Phase 6 entry below rather than a separate piece of work; listed here only because their file `mtime`s cluster in this window rather than July 27's.

### Date: July 27, 2026 | Status: Sprint 12 (Google Sign-In & Provider Lock) + Phase 6 (Advisory Tier Retirement & Product Tier Columns) — the two largest pieces of undocumented work this pass, shipped the same day. **Sprint 12** — `supabase/sprint12_provider_lock.sql` (new `user_preferences.signup_method TEXT CHECK (... IN ('magic_link','google'))`, backfilled to `'magic_link'` for every existing row); `app/api/auth/route.ts` (magic-link send) gains a provider-lock pre-check — if an email's `signup_method` is already `'google'`, returns `409 wrong_provider` instead of sending a doomed link; `app/api/auth/link-sessions/route.ts` gains the real backstop (step 0, runs for both auth directions) — if a just-completed auth forked a duplicate account for an email already locked to the other method, deletes the just-created duplicate auth user server-side and returns `409 provider_mismatch`; `app/auth/callback/page.tsx` determines `authMethod` from `user.app_metadata.provider` (never trusts the client) and renders a dedicated "you already have an account" state with a one-click retry in the correct method. Full mechanism (why two enforcement points are needed, not just one) in TSD §6.3. **Phase 6 — Advisory retirement:** `supabase/retire_advisory_tier.sql` drops the now-unused `advisory_access_requests` table (the Sprint M7 "request Advisory access" queue) — the actual feature-folding happened in application code across `components/SessionReliabilityIndex.tsx`, `components/ContradictionDetector.tsx`, `components/DecisionRules.tsx`, `app/api/mirror/rules/route.ts`, `app/api/mirror/graph/route.ts`, `app/mirror/page.tsx` (several of these show up with a July 28 `mtime` in the entry above — same change, filesystem just dates them a few hours later). Advisory's five former advantages (Peer Benchmark, SRI's "Next Move" callout, full Contradiction Detector detail, and the session-count bypass on Rules/Contradictions/Graph) are now just part of Elite; `access_type = 'advisory'` survives only as a provenance marker (manually granted vs. self-serve Razorpay) and still drives non-cancellable-via-self-service handling. **Same day, tier infrastructure:** `supabase/add_product_tier_to_mirror_access.sql` adds `mirror_access.product_tier` (`'elite'|'private'`, default `'elite'`) and `.private_model_family` (`'qwen'|'mistral'`); `middleware.ts` (new) resolves tier + TD-LD-10/11 routing override once per request and stamps it onto headers for `lib/ai-client.ts` to read, with zero changes needed to any of the 15 existing AI call sites; `lib/product-tier.ts` (new) and `lib/types.ts` extended with `ProductTier`/`PrivateModelFamily`/`RouteOverride`; `app/api/admin/grant-mirror-access/route.ts` extended to accept `productTier`/`privateModelFamily`/`modelRouteFast`/`modelRoutePremium` on top of its existing grant fields; `app/api/account/route.ts` updated (deletion route drops the now-gone `advisory_access_requests` reference — see its own comment, dated as a "Phase 6" cleanup). Full tier/routing mechanism in TSD §9.2/§9.4/§9.5. **Flagging as KDD-worthy:** zero paying users exist on either Elite or Private as of this migration (per the migration's own comment), so none of this has been exercised against real billing data yet — worth a deliberate smoke test before the next real signup lands on any tier.

**Same window, one more previously-undocumented fix:** `lib/supabase.ts`'s browser-side `createClient()` changed from constructing a fresh Supabase client on every call to a lazily-initialized module-scope singleton (`browserClient`). Every `'use client'` component was independently spinning up its own `GoTrueClient` against the same `localStorage` key (unsupported per Supabase's own docs) — with nothing listening for `onAuthStateChange`, calling `signOut()` on one instance had no reliable way to reach the others, producing an intermittent "sign-out doesn't take effect" bug. Plausibly what actually surfaced this fix: the same window added several new client-side auth entry points (`AuthPanel`, `EmailCaptureCard`, mirror's `AuthGate`), each now hitting `createClient()` on mount. Server-side callers unaffected — no `localStorage` to share across requests, so a fresh instance per call there was never the problem. Full detail in TSD §6.3.

### Date: July 26, 2026 | Status: `lib/tier-context.ts` (new) + payment webhook / `PaymentButton.tsx` touch — `tier-context.ts` is the `AsyncLocalStorage`-based per-iteration tier override for cron/batch routes (`daily-nudge`, `reanalyze-email`), which `middleware.ts` (July 27, above) structurally cannot cover since one trigger request fans out to many different users. **Not yet wired into either cron route** — flagged in the file's own doc comment as an open follow-up, since each route's actual call chain needs tracing first before wrapping it. Precedence is explicit in the code: a `tier-context` override always wins over `middleware.ts`'s header-based tier, so a batch route's per-user wrap can never be shadowed by whatever tier its own trigger request happens to carry. `app/api/payment/webhook/route.ts` and `components/PaymentButton.tsx` touched the same day — appear to be the ₹2,999/₹29,999 pricing-copy update (see the pricing note above) rather than a logic change; the webhook's own upsert doesn't set `product_tier` explicitly, so it continues to take the column default (`'elite'`) introduced the next day. Full tiered-routing mechanism in TSD §9.3.

### Date: July 21–25, 2026 | Status: Phase 3 batch (Pushback Classification, Mind-Change/Advisor-Divergence, Tag-Wiring Regression Tests, Decision Continuity/SessionView refactor) — **already fully documented in the Technical Spec's §2.19/§2.20/§3.19**, written from a same-or-later snapshot of this codebase and marked "not previously documented" there; not re-derived here. Files in this window: `lib/decision-continuity.ts`, `lib/mind-change-patterns.ts`, `app/api/pushback-classification/route.ts`, `app/api/mirror/mind-change/route.ts`, `lib/advisor-divergence.ts`, `components/MindChangeTile.tsx`, `components/MirrorEchoCard.tsx`, `components/TrustBadgeStrip.tsx`, `lib/council-context.ts`, `lib/persona-relevance.ts`, `components/WhatChangedDrawer.tsx`, `app/methodology/page.tsx`, `components/ReanalyzeDrawer.tsx`, `app/api/session/[id]/synthesis-summary/route.ts`, `components/DecisionStateCard.tsx`, `app/session/[id]/page.tsx`, `components/SessionView.tsx`, `components/PersonaPanel.tsx`, `lib/brief-markdown.ts`, `components/SynthesisCard.tsx`, `lib/decision-patterns.ts`, `lib/mirror-fingerprint.ts`, `components/RecordExport.tsx`, `app/api/session/[id]/brief/route.ts` (or equivalent), `app/api/session/[id]/observation/route.ts`, `lib/personas.ts`, `app/api/persona/route.ts`, and two new Vitest suites (`tests/tag-wiring-guardrail.test.ts`, `tests/persona-tag-wiring-guardrail.test.ts`). See TSD §2.19 for the pushback-classification/personalization mechanism, §3.19 for the Mirror-page and in-session surfaces (including two previously-undocumented pre-existing siblings the TSD's own author turned up while writing this section — `EarlyEchoCard.tsx` and its `echo-hint` route, both predating Phase 3), and §2.20 for the regression-test suite closing the five-sink-file tag-duplication gap this document and the TSD have both flagged repeatedly.

### Date: July 20, 2026 | Status: Institutional Tech Debt Fixes — **already fully documented in the Technical Spec's §12.11–§12.17**; not re-derived here. 12 files/1 migration: `supabase/institutional_tech_debt_fixes.sql`, `app/api/admin/create-institution/route.ts`, `app/api/institutions/[institutionId]/admin/{aggregate-dashboard,request-deactivation,rollup-dashboard,roster}/route.ts`, `app/api/institutions/consent/route.ts`, `app/institution/admin/page.tsx`, `components/CreateInstitutionPanel.tsx`, `lib/aggregate-benchmark.ts`, `lib/bias-parameter-benchmark.ts`, `lib/cohort-insights.ts`, `tests/institutional-tech-debt-fixes.test.ts`. This is the tech-debt-fix pass referenced in the TSD's schema-tracking note and its §12.14 (`toggle_consent()`, `get_user_emails()`, five `aggregate_read_*` wrapper functions) — closing out items in the fix ledger at §12.17. Same pass also closed Tech Debt #8 by *correction* rather than code change (the account-deletion cleanup it flagged as missing was already present, dated July 10) — see the TSD's own correction note in §1.

> **Note on the entries directly below (July 17 → July 8, 2026):** these were reconstructed the same way as the July 7 → June 25 block further down — by diffing the uploaded codebase against the last documented baseline — with one difference worth flagging: **no `.git` directory was present in this delivery** (the zip appears to be a clean export, not a repo clone), so there was no commit log to date against this time. Dates below are inferred from filesystem `mtime` on each file inside the extracted zip instead. Spot-checked against the one file with an independent confirmation available — `lib/decision-continuity.ts`'s mtime reads June 22, 2026, matching the RET-5 dated entry and KDDs 196–199 already in this document to the day — so the mtime approach is corroborated as reliable at least once, but treat day-level grouping here as somewhat softer evidence than the git-log-derived entries below it. If you have the actual commit history (locally or on GitHub), it would be worth re-dating this block properly at some point. As before: no new KDD/TD numbers assigned in this pass; several items below (Sprint CAL's clamp interaction with the K-floor-gated institutional context block, the `consent_aggregate_backfill` no-op, the `aggregate_reader` role never being wired in) read as KDD-worthy and are flagged for you to number.
>
> **Also worth naming directly: this entire block is Institutional Mode** — a complete B2B layer (six sprints + a tier-3 deactivation patch) that doesn't appear anywhere earlier in this document. It was evidently built and shipped without a handover session covering it. Full architectural detail — schema, K-floor mechanism, RBAC, consent model, all three aggregate view types, cohorts, admin console — is now in the Technical Spec document's new §12; this block stays in this doc's usual narrative/KDD-flagging register rather than re-deriving that detail.

### Date: July 17, 2026 | Status: Sprint CAL (Personal Calibration-Zone Boost) + P1 Gap #2 (Deliberation-Shift Boost) — both wired into `lib/persona-relevance.ts` the same day, roughly 2.5 hours after the P1 Worth Confirming/synthesis-diff batch below, touching the same file both times. **Sprint CAL:** when the current decision is elevated on an ontology dimension where the specific user has a confirmed personal calibration pattern (`lib/calibration-engine.ts`, pre-dates this window), every persona already mapped to that dimension in `DIM_PERSONA_BOOSTS` gets an additional flat `CALIBRATION_ZONE_BOOST = 0.10` on top of whatever rule/dimension boosts already applied — deliberately smaller than a primary boost (those run ~0.10–0.30), since this is meant to refine an existing signal, not introduce a new one. Explicit in-code note: rule-engine thresholds themselves are never personalised by this — personalisation stays confined to relevance weighting and the synthesis context block, a rule either fires or it doesn't, identically for every user. **P1 Gap #2:** a persona whose stated lean changed between the previous synthesis version and this one (`leanShifts`, computed by `lib/synthesis-diff.ts` — see the entry below) gets a flat `DELIBERATION_SHIFT_BOOST = 0.15`, not scaled by "how far" the lean moved (proceed/wait/mixed has no principled distance metric to scale against). This is also a genuine bugfix: before `leanShifts` existed as an input to this function, every other input was fixed at session-creation time, meaning re-running relevance scoring on every re-synthesis within a session always returned identical numbers regardless of how the actual deliberation evolved — nothing about the weighting could ever move mid-session. Both boosts stack additively onto the existing `0.50` base + rule/dimension-boost model (this is **not** the normalised-sum-to-1 formula this document's own earlier Technical Spec cross-references described — that formula never matched the implementation; corrected in the TSD this pass, see its §2.8). `app/api/persona/route.ts` and `components/SynthesisCard.tsx` both also touched in this exact window — the route now threads `personalCalibrationZones` and `leanShifts` into `computePersonaRelevance()`, and `SynthesisCard.tsx` picks up whatever changed in the `X-Persona-Relevance`/`X-Persona-Relevance-Reasons` headers as a result. **Flagging as KDD-worthy:** the interaction between this boost and the institutional synthesis-context injection (below) is untested — both are additive layers touching the same synthesis prompt, in different places, and nobody has verified what a user who is both in a personal calibration zone AND an institution member with cleared-floor data actually receives. 3 files touched, 0 schema changes, 0 new env vars.

### Date: July 17, 2026 | Status: Sprint 1 Follow-On — Worth Confirming (merged Features #1 + #6) + P1 (Synthesis Versioning / "What Changed") — 4 files, all touched within the same minute per mtime (`lib/worth-confirming.ts`, `lib/synthesis-diff.ts`, `components/WhatChangedDrawer.tsx`, `components/CouncilWeightingStrip.tsx`), continuing from the synthesis-version persistence route shipped the day before (see July 16 entry). **Worth Confirming (`lib/worth-confirming.ts`, `getWorthConfirmingText()`):** closes the synthesis with one quiet, optional line naming the single highest-value unknown behind the verdict — either an Examiner gap the user never actually answered, or (if no unanswered gap exists) the lowest-confidence ontology dimension that's also feeding one of the top-weighted persona relevance scores. Returns nothing — not a placeholder, an actual absence — when no clear candidate exists; explicitly not a manufactured hedge. Delivered via a new `X-Worth-Confirming` header on the synthesis call, `encodeURIComponent`-wrapped the same way the persona-relevance headers already were. **Synthesis Versioning (`lib/synthesis-diff.ts` + new `synthesis_versions` table + `app/api/session/[id]/synthesis-version/route.ts`, the last of which actually shipped July 16 — see below):** every synthesis generation, including the first, now writes a version snapshot (`verdict`, `tension`, per-persona `persona_leans`, `persona_relevance`). `diffSynthesisVersions(previous, current)` computes `verdictShift` (`held`/`flipped`/`mixed`), `leanShifts` (feeds directly into Sprint CAL above — same object shape, computed once, consumed twice), and per-persona `relevanceDeltas`. `components/WhatChangedDrawer.tsx` (NEW) renders this — gated on ≥2 version rows existing for the session, so a first-pass never-reanalyzed decision shows no trace of the feature. Worth noting this is a distinct trigger from RET-5's session-level "Reanalyze" (June 22) — a pushback-triggered re-synthesis *within one session* also writes a new version row, so this can fire without ever touching the Decision Arc timeline. `components/CouncilWeightingStrip.tsx` also touched in this batch — presumably wired to read the new `X-Worth-Confirming` header and/or reflect the relevance-reasons header, but no in-code comment narrates the specific change; functional description only, not confirmed against a diff. **Flagging as KDD-worthy:** neither `lib/worth-confirming.ts` nor `lib/synthesis-diff.ts` carries a sprint codename or KDD reference in its own header comment (unusual for this codebase — most files in this era self-document with a sprint tag) — "Sprint 1 follow-on" and "P1" are both taken from a single inline comment in `app/api/persona/route.ts` (`// Sprint 1 follow-on (merged Features #1 + #6)`, `// P1: Gap #2 leanShifts typing`) rather than from the files themselves. 4 files new/changed, 1 new table (`synthesis_versions`), 1 new response header (`X-Worth-Confirming`).

### Date: July 16, 2026 | Status: Examiner Resolvability Check (Audit Fix #3) + Synthesis-Version Persistence Route — Two unrelated pieces, same day. **(1) `lib/examiner-resolvability-check.ts` (NEW) + `app/api/examiner/route.ts`:** before committing to a REDIRECT for rule **R7 specifically** ("decision needs information you don't have yet" — R1, the other REDIRECT-eligible rule, is untouched by this), the route now calls `checkR7Resolvable(decisionText, infoRationale)` — one cheap AI check asking whether a genuinely useful, honest, *provisional* answer is possible today even with the gap named openly. If yes, the mode is downgraded from REDIRECT to GATE in place before the response is built, with the rationale logged via `console.log` for traceability — R7 then follows the same clarifying-question path every other gated rule uses instead of a hard stop. In-code comment labels this "recommendation #3" from an unnamed audit — the other two recommendations from whatever that audit was aren't identifiable anywhere else in this codebase; worth asking whoever ran it (or checking your own notes) whether #1 and #2 are still outstanding. **(2) `app/api/session/[id]/synthesis-version/route.ts` (NEW):** the persistence half of the What Changed feature (see the July 17 entry above for the read/diff half, which shipped the following day) — `POST` writes a `synthesis_versions` row, auto-incrementing `version_number` per session. Shipped in the morning (04:04 UTC by mtime), several hours before the resolvability check (11:52 UTC) — genuinely two separate pieces of work on the same day, not one commit. 2 files new, 1 file modified, 0 schema changes beyond the new table already counted above.

### Date: July 14, 2026 | Status: Institutional Mode — Redemption, Admin Console UI, Tier 3 Deactivation ("ship it" day) — By far the largest single-day batch in this reconstruction: 11 files touched within a 52-second mtime window (04:15:50–04:16:42 UTC), following roughly a week of schema/library groundwork (see the July 7–12 entries below this one). This is the day the institutional layer actually became reachable by a human rather than existing only as schema and unwired library functions. **New pages:** `app/institution/join/page.tsx` — the first (and only) UI path to `POST /api/institutions/redeem`, which had existed since July 7/8 with nothing linking to it. `app/institution/admin/page.tsx` (657 lines) — single-page admin console, no tab routing, loading roster/codes/consent-changes/consent-rate/aggregate-dashboard/cohorts/rollup-dashboard on mount. **New/changed routes:** `POST /api/institutions/redeem` (redemption, admin-seat first-claim race handled via conditional `UPDATE ... WHERE admin_seat_claimed = false`), `POST /api/admin/create-institution` (platform-admin-only, `ADMIN_CODE`-gated, not reachable by an institution's own admin — this is also where the `deactivated_at` toggle from `institutional_tier3_deactivation.sql` actually gets exercised), `GET/POST .../admin/cohorts` + `POST/DELETE .../admin/cohorts/[cohortId]` (admin cohort CRUD — creation and population deliberately kept as separate actions), `GET .../admin/rollup-dashboard` (parent-institution-only, reads the Sprint 4 rollup view for the first time from any route), `GET /api/institutions/bias-parameter-benchmark` (member-facing wrapper for the Sprint 6 view), `GET /api/institutions/consent/history` (caller's own consent audit trail), plus `lib/bias-parameter-progress.ts`. **`supabase/institutional_tier3_deactivation.sql`:** single nullable column, `institutions.deactivated_at` — not an `is_active` boolean, not a hard delete. Blocks new redemptions against a deactivated institution's code; does not touch existing members, consent, or data. Migration's own comment notes a genuine hard-delete would need its own explicit decision — this was deliberately scoped down to soft deactivation only. **Flagging as KDD-worthy:** there is no institution-admin-facing self-deactivation path anywhere — only the platform operator (`ADMIN_CODE`) can deactivate an institution. If that's intentional (plausible — deactivation has billing/relationship implications an institution's own admin probably shouldn't unilaterally trigger) it's worth a KDD saying so explicitly, since it reads like a gap otherwise. 11 files new/changed, 1 schema change (`institutions.deactivated_at`).

### Date: July 12, 2026 | Status: Institutional Sprint 6 (Bias Parameter Benchmark, per SQL filename — **not** your planned Sprint 6 "Hardening, Edge Cases, Rollout," which per `TECH_DEBT.md` hasn't happened yet — see the naming note added to TSD §12.9 this pass) + Cohort Insights UI — `supabase/institutional_sprint6_bias_parameter_view.sql` + `lib/bias-parameter-benchmark.ts` + `components/CohortInsightsCard.tsx`. Sprint 6 is a second, separate aggregate-view type from Sprint 4 below — deferred out of that earlier sprint by design because it needed a different join path: Sprint 4's views key on the 14-dimension ontology vocabulary, but `bias_library` is keyed by `bias_parameter`, a different vocabulary. New views: `institutional_platform_bias_parameter_segments`, `institutional_bias_parameter_segments` — per `bias_parameter`, `member_count` + `avg_confidence_weight`, same K-floor gating as the Sprint 4 views. **A genuine cross-cutting gap this join surfaces, not introduced by it:** `bias_library` is keyed by `user_email`, not `user_id` — a pre-institutional design choice, per that table's own original migration comment ("identifier until auth is added"). This view bridges the two via a join through `auth.users.email`; a user whose `bias_library` rows were written under an email that no longer matches their *current* login email will silently fall out of this join. Same identity-continuity gap that exists anywhere else `bias_library` is read by current session identity — just newly documented here because this is where it was actually noticed. **Deliberately excluded from the view:** `activation_contexts` (can carry decision-specific free text per its own column comment) and `detection_count`/`asymmetry_score_avg` — kept out of an aggregate/anonymized view on purpose. `components/CohortInsightsCard.tsx` (NEW, same day) — renders on a member's own Mirror page as "Your Cohort," showing mutual-consent cohort peers' `sessionScore`, `sessionScoreDelta`, `calibrationDeltaAvg`, and confirmed bias labels — never raw decisions or session content from either side. Renders nothing at all (no empty-state UI) if the member has no cohort with at least one other mutually-consenting peer. **Worth flagging directly: this card is not gated on `mirror_access`/paid Mirror tier** — reachable by any authenticated institution member regardless of whether they personally hold a paid Mirror subscription (see the July 14 institutional-mode wrap-up note in PENDING/NEXT SESSION below for the billing-model implication of this). 3 files new/changed, 2 new views.

### Date: July 11, 2026 | Status: Institutional Sprint 4 (Aggregate Benchmark Views) + Unlock Progress + Admin Consent-Rate/Aggregate-Dashboard — `supabase/institutional_sprint4_aggregate_views.sql` (the actual views — the restricted-role half of Sprint 4 shipped two days earlier on July 9, meaning the narrower DB role was defined before the views it was meant to protect existed) + `lib/unlock-progress.ts` + two new admin routes. **What's actually being measured — worth being precise about, since "benchmark" undersells it:** this is a population-level version of the existing personal Calibration Engine, not a structural-similarity/"what did others decide" comparison. For each of the 14 ontology dimensions, sessions are bucketed `HIGH` (score ≥4) or `LOW` (≤2) — same threshold convention as the personal engine — and average `outcomes.calibration_delta` is compared between buckets; `gap = high_avg − low_avg`, `is_signal = |gap| ≥ 0.4` (same `MIN_GAP` constant as the personal version, not independently chosen). Two views: `institutional_platform_benchmark_segments` (platform-wide, deduplicated by `user_id` before joining session data — a user who consents in more than one institution would otherwise be double-counted) and `institutional_benchmark_segments` (per-institution, using that institution's own `k_floor_override`). `getBenchmarkForDimension()` tries the institution-scoped view first, falls back to platform-wide if the institution is unset or below floor, returns `{ scope: 'insufficient' }` if both fail — every number returned carries this scope tag so the UI never presents an unscoped number as unscoped truth. `lib/unlock-progress.ts` — when a dimension comes back `insufficient`, computes and returns how far the relevant population is from clearing the floor ("N more consenting members needed") rather than a bare no-data response. Admin routes: `GET .../admin/consent-rate` (deliberately gated on **total membership** clearing the floor, not the consenting count — gating on the consenting count would leak nearly the same information the gate exists to hide) and `GET .../admin/aggregate-dashboard` (every dimension that's cleared this institution's floor, one query against the view rather than 14 calls to `getBenchmarkForDimension()`). 4 files new/changed, 2 new views.

### Date: July 10, 2026 | Status: Institutional Sprint 5 (UI Support: Active-Institution Preference + Unlock Notices) — `supabase/institutional_sprint5_ui_support.sql` + `lib/active-institution.ts` + `lib/unlock-notices.ts` + `app/api/institutions/active/route.ts` + `app/api/institutions/benchmark/route.ts` (the member-facing wrapper around Sprint 4's `getBenchmarkForDimension()`, shipped here rather than alongside Sprint 4 itself — reachable by any authenticated user, institution member or not; a non-member is simply always routed to the platform-wide tier). Two new small tables: `user_institution_preference` (one row per user, `active_institution_id` — backs "which institution's context am I currently viewing" for a user belonging to more than one, deliberately a dedicated table rather than `localStorage` so the choice carries across devices like any other account setting) and `seen_unlock_notices` (one row per `user × dimension × scope_type`, powering a one-time "this benchmark just unlocked" toast that never re-fires — institution-level and platform-level unlocks for the same dimension are tracked as genuinely separate events). 5 files new/changed, 2 new tables.

### Date: July 9, 2026 | Status: Institutional Sprint 3 (Cohorts) + Sprint 4 Restricted Role (schema only) + Admin Roster/Codes/Role Routes — Two threads, same day. **Sprint 3:** `supabase/institutional_sprint3_cohorts.sql` — `cohorts` + `cohort_memberships` tables. Worth noting for context: Sprint 1 (July 7–8) originally put a single `cohort_id` column directly on `institution_memberships`, anticipating one cohort per membership; this migration drops that column in favor of the proper many-to-many join table once the actual plan called for a user belonging to more than one cohort. `lib/cohort-insights.ts` (NEW) — mutual-consent-gated cohort peer insights (both sides must have `consent_shared_cohort = true`; a member consenting alone sees no peers and isn't shown to anyone who hasn't also consented), `app/api/institutions/cohort-insights/route.ts` (member-facing wrapper). **Admin routes, same day:** `GET .../admin/roster` (membership metadata only — `userId`, `email`, `role`, `joinedAt`, no consent flags, no session/bias data; **flagged in the route's own comments as an N+1 pattern** — no bulk "get users by ID" in `supabase-js`, so email resolution is one `auth.admin.getUserById()` call per member, fine at pilot scale, worth replacing with a Postgres join before a large institution), `POST .../admin/codes` (rotation + child-institution creation for the conglomerate hierarchy — does not reset `admin_seat_claimed`, that flag is specifically about who won the *original* first-ever redemption), `POST .../admin/role` (RBAC assignment — refuses to demote an institution's last remaining admin, `409`, since zero admins would permanently lock the institution out of its own admin routes). **Sprint 4 restricted role (schema half only):** `supabase/institutional_sprint4_restricted_role.sql` defines a narrower Postgres role (`aggregate_reader`, intended to read only the aggregate views rather than the blanket access `service_role` has) plus `lib/k-floor.ts` (`DEFAULT_K_FLOOR = 20`, `effectiveKFloor()`). **As of this reconstruction, nothing in the application code has been switched to use the restricted role** — every institutional route still calls `createServiceClient()`, the same full-access client used everywhere else. The role is defined in the database; nothing reads through it. **Flagging as KDD-worthy and worth prioritizing**, not just noting — this means the current blast radius of any bug in the aggregate-serving code path is "full service-role access to every table," not the narrower surface this role exists to provide. 6 files new/changed, 2 new tables, 1 new DB role (defined, unused).

### Date: July 8, 2026 | Status: Institutional Sprint 2 (Consent + Audit Log) + RBAC Foundation — `supabase/institutional_sprint2_consent_audit.sql` — `consent_audit_log` table, append-only (no update/delete path anywhere in the codebase for this table), plus the three consent booleans added to `institution_memberships`: `consent_aggregate`, `consent_aggregate_backfill`, `consent_shared_cohort` — three independent flags, not one. `lib/institution-auth.ts` (NEW) — `requireInstitutionRole(req, institutionId, allowedRoles[])`, the shared guard every institutional admin route calls first (resolves Bearer token → user → membership row → role check); this is the first real caller of this exact pattern in the codebase, everything institutional built after this reuses it rather than reimplementing its own check. `lib/aggregate-eligibility.ts` + `lib/cohort-sharing-fields.ts` (both small, shared-constant files). `POST /api/institutions/consent` (toggles the caller's own consent flags, writes a matching audit-log row in the same request) + `GET .../consent-changes` (admin-only — **the query only ever selects `field_changed` and `changed_at`, never `user_id`**, enforced by the query shape itself rather than just response filtering; there is deliberately no RLS SELECT policy granting an admin row-level access to `consent_audit_log` at all). **Worth flagging now, not just in the TSD:** `consent_aggregate_backfill` is stored and settable from day one, but no aggregate view built in any of the sprints after this one actually branches on it — every view filters purely on `consent_aggregate = true`, with no distinct backfill behavior. As written, granting `consent_aggregate` includes a user's full qualifying history (past and future) immediately; the backfill flag currently does nothing distinct. Worth a decision — implement the distinction, or drop the column — before this is presented to a real institutional user as if it does something. 6 files new/changed, 1 new table, 3 new columns.

### Date: July 7, 2026 | Status: Institutional Sprint 1 (Schema Foundation) — `supabase/institutional_sprint1_schema.sql`, same day as Sprint R6 below but no file overlap (confirmed — R6 touches `app/api/persona/route.ts`/`components/PersonaPanel.tsx` only). `institutions` table: `id`, `name`, `parent_institution_id` (self-referencing FK — this is what eventually enables the conglomerate/rollup hierarchy, Sprint 4/July 11) `unlock_code_hash` (SHA-256; the raw code is never stored, shown once at creation/rotation and never retrievable again), `admin_seat_claimed` (flips atomically on first redemption), `k_floor_override`, `allowed_email_domains`. `institution_memberships` table: one row per `(user_id, institution_id)`, unique constraint on the pair, `role` (`admin`/`member`, default `member`). Both tables RLS-enabled with SELECT-only policies for `authenticated` — deliberately **no INSERT policy for the authenticated role on either table**, membership and institution rows are created exclusively by service-role routes, never directly by the client (this remains true through every later institutional sprint — confirmed by grep across the full set). This is schema only — no route or UI exists yet to reach any of it; that starts the next day (July 8). 1 file new, 2 new tables.

 (Structural Citation → Tag, Not Free-Form Sentence) — `app/api/persona/route.ts`, `components/PersonaPanel.tsx`. R5 (pre-dates this window) had personas close with a conditional free-form sentence ("Structurally, this decision...") when structural retrieval fired — but the frontend never parsed it; it streamed in as anonymous prose, indistinguishable from the rest of the analysis. The only visible signal that structural retrieval had fired at all was a single hardcoded banner permanently pinned to the Pattern Analyst card, driven by a session-wide flag — disconnected from whether Pattern Analyst, or any of the other 4 personas in `PERSONAS_WITH_STRUCTURAL_CONTEXT`, had actually produced a citation that session. Fix: the sentence is now wrapped in `<structural>...</structural>`, matching the existing `<lens>`/`<position>`/`<realcost>`/`<lean>` header-tag convention. `PersonaPanel.tsx`'s `extractHeaderTags` extracts and strips it per-persona; a citation badge renders only on cards whose own output actually contains the tag — no more one session-wide flag standing in for five personas' individual editorial judgment. 2 files changed. This is the third addition to a tag family (`<lean>`/`<wait>`, `<verdict>`/`<tension>`, now `<structural>`) that has leaked or malformed on the frontend more than once historically — no shared sanitiser exists yet; each new tag still needs its own strip pass wired at every consuming surface.

### Date: July 5, 2026 | Status: Sprint W1 (Watchlist) — 6 new files: `app/api/watchlist/route.ts` (GET/POST), `app/api/watchlist/[id]/route.ts` (PATCH/DELETE), `app/api/watchlist/[id]/graduate/route.ts` (POST), `components/WatchlistSection.tsx` (home page), `lib/feature-flags.ts` (`isWatchlistEnabled()`). Gated end-to-end on `NEXT_PUBLIC_WATCHLIST_ENABLED === 'true'` — off by default; all 4 routes return `404` (not an empty result) when the flag is off, so the disabled feature looks absent rather than broken. Flag is `NEXT_PUBLIC_`-prefixed so it reads identically from server routes and client components — but that also means it's inlined into the client bundle at build time; a Railway restart alone will not pick up a flag change, a rebuild is required. Deliberately low-friction and distinct from a full Council session: one text box, optional tag (`business | wealth | career | family | relationship | other`), open items each with Archive (soft, `status='archived'`) and hard Delete actions, plus a "Convene the Council" graduate action. Capture text encrypted at rest (`text_encrypted`). All routes Bearer-authenticated only — no anonymous path, since this writes state (same posture as the graph-nudge route below). New table: **`watchlist_items`** — no migration file found in the repo (see schema-tracking note added to the TSD's DB table list); confirm this table actually exists in the live Supabase instance before flipping the flag on in production, or re-run whatever SQL created it. 6 files changed, 1 new table (unconfirmed migration provenance), 1 new env var (`NEXT_PUBLIC_WATCHLIST_ENABLED`).

### Date: July 5, 2026 | Status: Sprint M7 (Mirror Advisory Request Flow) — 1 new route: `POST /api/mirror/advisory-request`. `components/AdvisoryUpsellCard.tsx` gets the request CTA wired to it (the upsell card itself pre-dates this window). Lets a user request Advisory-tier access as an alternative to self-serve Mirror checkout — reads as a manual/sales-assist request-logging endpoint rather than an automated grant, consistent with Advisory being a higher-touch tier elsewhere in the spec (`ADVISORY_BYPASSES_THRESHOLDS`). No further rationale found in code comments for this file — functional description only.

### Date: July 4, 2026 | Status: Sprint QW-3 (Post-Session Graph Nudge) — 2 new files: `app/api/session/[id]/graph-nudge/route.ts`, `components/GraphNudgeLine.tsx`. Powers a single shared end-of-flow prompt slot in `SessionView.tsx` — deliberately not a re-render of the full `DecisionGraph` d3 component (that already has a home in Mirror by session 6+; this slot only needs to say "something changed there," not show it again). Gated to `sessionCount ≥ 6`, mirroring server-side the client's own gate that reserves the pictorial 1–5-session graph teaser (Sprint S1-07, pre-dates this window) for newer users. Three mutually-exclusive variants share ONE 72-hour cooldown (`user_preferences.last_graph_nudge_shown_at` — whichever variant fires updates the same timestamp): **(1) `milestone`** — veteran users (20+ sessions) only, fires when total edge count crosses a new rung in `[10, 25, 50, 100, 250, 500, 1000]`. **(2) `new-connection`** — non-veterans, fires when a `graph_edges` row touching *this* session is newer than the last time this slot was shown. **(3) `watchlist-suggestion`** — fallback when neither graph variant fires and the Watchlist flag (Sprint W1, above) is on: offers to add this session's first non-empty `examiner_gap_1/2/3` phrase to the Watchlist, sourced from data the Examiner already computed for this exact session — no new detection pass run just to manufacture a prompt. Graph variants always win over the Watchlist fallback when both are eligible. Auth: resolved `user_id` required, no anonymous fallback — this route writes state. 2 files changed, 0 schema changes (reuses `user_preferences`, `graph_edges`).

### Date: July 3–5, 2026 | Status: Sprint QW-2 (Decision Graph — Tiered Access, Was Single Unlocked-Only Gate) — `app/api/mirror/graph/route.ts` rewritten. Before this sprint, graph access was all-or-nothing: Mirror-unlocked AND a 20-session/3-edge corpus, or nothing. Two real problems fell out of that — free users never saw the product's most differentiated asset before paying, and *paying* subscribers under the 20-session corpus saw an empty graph too, which is a retention bug, not a deliberate paywall. Split into three tiers: **`locked`** (< 2 sessions) — at most a single self-node, a "your first decision is mapped" ghost state. **`preview`** (≥ 2 sessions, not yet paid+corpus) — real edges are returned (the fact that a connection exists is the hook) but `dimension_breakdown`/`explanation_text`/`metadata` are stripped server-side per edge — the interpretive "why" stays the paid layer, same principle as the existing `ADVISORY_BYPASSES_THRESHOLDS` contradiction-detail gating — capped to 2 edges (strongest first), remainder surfaced only as a count. **`full`** (unlocked + Advisory bypass or corpus met) — unchanged original behaviour. `MIN_GRAPH_SESSIONS`/`MIN_GRAPH_EDGES` now gate only the preview→full transition, for paying users only, defaults lowered from 20/3 to **2/1** — a paying subscriber shouldn't need 20 sessions to see their own graph in full (Railway env vars still set to the old 20/3 defaults from before this sprint continue to apply until changed there). Response now carries an explicit `redacted: boolean` per edge so the client can distinguish "computed and genuinely empty" from "stripped because you're on preview" without guessing from nulls, and a tier-aware `corpus` block describing what's needed for the *next* tier up rather than one fixed threshold. **Flagging this as KDD-worthy** — it's a real product-tier redesign, not a bugfix, and isn't logged as a KDD anywhere I can find in-repo. 1 file changed, 0 schema changes, env var defaults changed in code (not yet reflected on Railway per the note above).

### Date: July 3, 2026 | Status: BUGFIX audit pass — persona-relevance scoring silently flat since ship; second, unrelated column-name bug found in the same pass — Two silent failures, same root cause pattern, found together during a July audit (the first surfaced indirectly via Sprint S2-02's Council Weighting Strip making relevance values visible for the first time — see the July 1 entry below). **(1) `app/api/persona/route.ts` — `fetchCouncilContext()`:** the Council-context query selected `matches_json` from `sessions_ontology` — but `matches_json` has never lived on that table; it's written to the separate `structural_matches` table by `/api/structural-match`. Including a nonexistent column in a Supabase select rejects the **entire** query (Postgres error 42703), so `data` came back `null` on every single call, silently — no thrown exception the route's existing error handling would catch. Effect: `maxStructuralScore` was always `null`, and `computePersonaRelevance()` (whose flat-0.50 no-data fallback is itself correct, intended behaviour — see the KDD 100 reference in the June 21 test-scaffold entry above) always hit that fallback for every persona, on every session, since this code shipped — the fallback path was firing when it shouldn't have been, not misbehaving itself. Fixed: `matches_json` now fetched via its own query against `structural_matches`, decrypted, joined in-memory with the `sessions_ontology` row; the old query's error is now logged via `console.warn` instead of silently swallowed, specifically so a future schema drift like this one surfaces immediately rather than silently degrading a scoring signal for weeks. **(2) `lib/session-score.ts`:** same `matches_json`/`sessions_ontology` mistake independently present in `scoreStructural()`'s query — structural sub-scoring for the Mirror's Session Reliability metric was also always falling back to its neutral default. Separately, the `bias_library` query in the same file used a column `bias_label` that doesn't exist — the real column is `bias_parameter` (confirmed against every other `bias_library` call site, e.g. `app/api/bias-score/route.ts`) — same failure mode, so bias-clarity sub-scoring was also always silently falling back to its neutral default (80). **(3) Separately, a genuine logic bug tracked in-code as `MIRROR-1`:** `scoreStructural()` previously returned a bare number, overloading `50` as both "no comparison data for this session" and a legitimately reachable real score; `deriveActionPlan()` checked `structural === 50` as its proxy for "user doesn't have 3+ decisions yet" — but a rounded average across up to 20 sessions can land on exactly 50 by pure arithmetic coincidence for a user with plenty of real data, which is what was producing "Bring 3 or more decisions" messaging for veteran users. Fixed by returning an explicit `hasData`/`structuralDataCount` flag instead of overloading the score, and checking that directly. 2 files changed. **Flagging both structural-source bugs as worth a shared KDD entry** — same failure class hit two independent files; may be worth a lightweight shared type/helper for "safe optional join to `structural_matches`" so a third instance doesn't recur silently a third time.

### Date: July 2, 2026 | Status: Sprint O3 (Decision-Maker Observation Auto-Surface) — 1 new route: `POST /api/session/[id]/observation`. Auto-surfaces the Decision Brief's "Decision-Maker Observation" line (the closing, second-person "how you make decisions" sentence from the `DECISION_BRIEF` prompt) directly below the synthesis card for Mirror subscribers, without requiring a click into "Generate Decision Brief" first. Gated on Mirror `unlocked` state — locked/teaser users get `{ observation: null }`, client renders nothing (the upsell already lives elsewhere on the synthesis card). First call: reads the earliest assistant message per persona plus the synthesis message, strips header/synthesis tags, sends to a dedicated lightweight prompt (`DECISION_OBSERVATION_PROMPT`, `lib/personas.ts`) — same register/constraints as the Decision Brief's own observation section, extracted as a standalone call so it can fire automatically. Result cached on `sessions.decision_observation`; repeat page loads return the cached value with no re-generation, no re-billing. 1 file changed (plus `lib/personas.ts` prompt addition), 1 new column (`sessions.decision_observation`).

### Date: July 1, 2026 | Status: "Enrichment Sprint" S2 (S2-01 Confidence Re-Rate, S2-02 Council Weighting Strip, S2-08 Reanalyze Synthesis Summary) + Sprint SB-3 (Council Context: Profile + Framing Intent + Correction Carry-Forward Injection) — Four related pieces shipped together, all feeding richer post-synthesis surfaces and richer Council input. **S2-01** — `PATCH /api/session/[id]/confidence`, `{ post_decision_confidence: 1–10 }`: post-synthesis confidence re-rate widget, counterpart to the existing pre-decision confidence capture, enabling before/after delta analysis. Idempotent by design: if already set, returns `{ ok: true, skipped: true }` without overwriting — a rating, once given, isn't editable via this route. Ownership resolved via the standard 3-tier precedence (`user_id` > `user_email` > `device_id`). **S2-02** — `components/CouncilWeightingStrip.tsx`, reading a new `X-Persona-Relevance` response header on the persona stream (the exact `relevanceMap` used to build the synthesis directive, exposed rather than recomputed client-side). Shown post-synthesis for every tier — it's explaining a synthesis the user already received for free, so gating it would be punitive. Top-3 advisors as a horizontal bar chart, only rendering at all if at least one persona scored above the 0.52 baseline. **This sprint's diagnostic work is what surfaced the persona-relevance bugfix logged under July 3, above** — exposing the exact weighting values for the first time made the flat-0.50-for-everyone pattern visible. **S2-08** — `GET /api/session/[id]/synthesis-summary`: returns this session's tag-stripped synthesis text in full for `components/ReanalyzeDrawer.tsx`, so a user reanalyzing a past decision can recall what the Council originally concluded before deciding what to change — full cleaned text returned, client renders a "Show more" toggle rather than the server truncating with no way back. **SB-3** — `lib/rule-engine.ts`'s `buildCouncilContext()` extended with three new optional parameters, all additive/backward-compatible: `profile` (injects a `WHO IS BRINGING THIS DECISION` block from `user_profiles`, instructing personas to calibrate angle/register from it without repeating labels verbatim — used most by Elder/Risk Architect/Contrarian/Pattern Analyst), `framingIntent` (`'challenge' | 'clarify' | 'right'`, captured at session creation — see the June 26 SB-1 entry below — each value carries its own persona directive), and `validationCorrection` (a `PRIOR SESSION CORRECTION` block asking personas to check whether the same corrected dynamic from a linked prior session is present here, named explicitly rather than left as a structural inference — closes the loop from SB-1's ValidationCard correction flow into the *next* linked session's Council context). 4 files changed (`app/api/session/[id]/confidence/route.ts` new, `components/CouncilWeightingStrip.tsx` new, `app/api/session/[id]/synthesis-summary/route.ts` new, `lib/rule-engine.ts` + `app/api/persona/route.ts` extended), 1 new column (`sessions.post_decision_confidence`), 1 new response header (`X-Persona-Relevance`).

### Date: June 28, 2026 | Status: Sprint S1-03 (Council Verdict & Tension Highlight) + Ontology Reveal / Session Labels — `components/SynthesisCard.tsx` gets a live-streaming token parser for two new tags: `<verdict>...</verdict>` — the synthesis's headline recommendation as a single sentence (client-truncated to the first complete sentence as a guard against the model over-writing), pulled entirely out of the prose and rendered in its own gold-accented box, mirrored in the Decision Brief PDF (`app/api/record/[id]/brief/route.ts`, theme-aware palette). `<tension>...</tension>` — a single sentence naming the central tension the synthesis resolves; unlike `<verdict>`, the *content* stays inline in the prose (only the tags are stripped), rendered with a highlighted background rather than pulled into a separate box — same treatment in the PDF. A small state machine (`parseModeRef`) buffers into `verdictAccRef`/`tensionAccRef` as the stream arrives so both populate progressively rather than only after the stream completes. Same day: `components/OntologyRevealCard.tsx` (new) and `lib/session-labels.ts` (new) — a shared plain-English label map (`DIMENSION_LABELS`, `DECISION_TYPE_LABELS`, `REVERSIBILITY_LABELS`, `FRAMING_INTENT_LABELS`, `getTopDimensions()`) used by the new reveal card, `SessionCompleteBadge.tsx`, and the Decision Profile Strip. **Note:** `lib/session-labels.ts` is unrelated to session-history-list titling despite the similar name — see the TSD §2.11 correction; that route returns raw `decision_text` with no generated label at all, a pre-existing spec/implementation mismatch unrelated to this sprint, corrected in this doc pass. 4 files changed/new.

### Date: June 27, 2026 | Status: Validation Nudge Cron + Shared Cross-Cron Notification Gate Rewrite + Session Bias Note — 3 new files. **`app/api/cron/validation-nudge/route.ts`** — `POST`, `0 2 * * *` (02:00 UTC), deliberately scheduled BEFORE daily-nudge's 04:00 UTC run so this source claims the shared notification slot first when both crons want the same user in the same window. Targets: authenticated sessions with `validation_state = 'pending'`, never nudged before (`validation_nudge_sent_at IS NULL`), 7–60 days old (7-day minimum spacing floor per the original ask, 60-day outer bound so it doesn't reach into ancient history), AND — this is the important guard — must have actually completed synthesis (a `messages` row with `persona='synthesis', role='assistant'` for that session), because REDIRECT-blocked sessions never run the Council and default to `validation_state='pending'` at creation with nothing to ever move them out of it; without the check they'd get nudged to validate an inference that was never made. One nudge per user per run (most recent qualifying session only). Reuses the 3 `validation_pending` copy variants added alongside SB-1 (see June 26 below); variant pick is deterministic per session ID, not per day, since this fires once per qualifying session rather than on a daily rotation. **`lib/notification-throttle.ts`** rewritten as a shared cross-cron gate (`canSendNudge()`/`recordNudge()`) — single combined decision across all nudge sources (daily-nudge, validation-nudge, and whatever else calls it) writing to `notification_log`, replacing what was previously a narrower, push-specific throttle; this is what lets validation-nudge's earlier schedule "win" the shared slot for a given user/day. **`app/api/session/[id]/bias-note/route.ts`** (new) — surfaces the single strongest *distorting* bias for this specific session (not a cross-session pattern), read from `bias_library.activation_contexts[sessionId]`, ranked by `prosecutor_score`; handles both identified sessions and anonymous ones via a synthetic `anon:<sessionId>` device key already written by the bias-score route. Powers `components/BiasNoteCard.tsx`. 3 files changed/new, 0 new schema beyond whatever `notification_log` migration this depended on (not found in repo — see schema-tracking note).

### Date: June 26, 2026 | Status: Sprint SB-1 (Framing Intent, ValidationCard v2, Profile Capture) + Sprint SB-2 (Examiner Rewrite — E0/S0/C0 Slot Architecture) — Two substantial pieces shipped the same day. **SB-1:** `framing_intent` (`'challenge' | 'clarify' | 'right'`) captured at session creation (`POST /api/session`), stored on `sessions.framing_intent`, each value carrying its own Council directive (fed into SB-3's council-context injection, see July 1 above). `components/ValidationCard.tsx` rewritten around the tiered signal system now documented in TSD §3.15 (tier matrix by session count, `buildValidationLine()`/`buildContextLines()`), with confirm/correct actions moved to `PATCH /api/session/validate` — a `'corrected'` submission stores the correction on `sessions.validation_correction`, which is what SB-3's carry-forward loop reads on the *next* linked session. `components/ProfileCaptureOverlay.tsx` (new) — optional, dismissible, on home + Mirror pages — lets a user self-identify `archetype`, `primary_fears[]`, `mbti_type` (validated client-side against the 16 real codes), `life_stage`, `risk_stance` via `POST /api/profile` into a new `user_profiles` row; only shows if at least one field is still unset. **SB-2 (Examiner rewrite, internally logged as "SB-2 v2.0" in route console output):** `app/api/examiner/route.ts`'s `GET` moved from a one-question-at-a-time polling loop (documented in the prior TSD, now corrected) to a single-call batch of up to 3 questions across three fixed slots — **E0** (always, non-REDIRECT: inward/emotional question surfacing the unnamed fear/identity dimension, informed by `primary_fears` + `dominant_emotion` when available), **S0-or-rule** (mutually exclusive: S0 domain-orientation question if the brief is under 25 words, otherwise the top-priority Rule Engine question), and **C0** (always: reflective close, sharpened against confirmed longitudinal bias patterns and — new this sprint — the SB-1 profile). REDIRECT mode (R1/R7) still overrides all three slots with a single generated resolution question. `POST` now accepts a full response batch or a `skipped` flag, either path firing three non-blocking triggers: `fireBiasScore()`, `fireIndependenceScore()`, `fireContradictions()`. A v1.0 gap-based fallback (`examiner_gap_1/2/3`) remains for sessions without ontology tagging. TSD §2.5 rewritten to match — the previously-documented one-at-a-time architecture doesn't match any commit in this repo's history that I could find; this whole slot system dates from this sprint. 5 files changed/new (`app/api/session/route.ts`, `components/ValidationCard.tsx`, `app/api/session/validate/route.ts`, `app/api/session/[id]/validation-signal/route.ts`, `components/ProfileCaptureOverlay.tsx`, `app/api/profile/route.ts`, `app/api/examiner/route.ts` — SB-1 and SB-2 together touch more files than usual for one date; listed together because both are dated to the same commit day, not because they're the same initiative), 1 new table (`user_profiles`), 2 new columns (`sessions.framing_intent`, `sessions.validation_correction`).

### Date: June 25, 2026 | Status: Sprint TOUR-1 (Onboarding & Guided Tours) — 2 new files: `components/OnboardingTour.tsx` (home page), `components/RecordTour.tsx` (record page); a third tour ("council tour") added inline in `components/SessionView.tsx` rather than as a separate file. Lightweight, dismissible, step-based walkthrough overlays, each with its own independent dismissal/seen-state flag: home tour fires once after the input form is revealed (auto-advances from the intro panel into the input-field reveal after a fixed 1800ms delay rather than waiting for a click, then proceeds step-by-step from there); council tour fires once after synthesis completes on a user's first session; record tour wraps the record page as a client component, consistent with the existing Server Component constraint (page-level data fetching stays server-side; the tour overlay is client-only). A user who dismisses one tour still sees the other two on their respective first visits. 2 new files, 1 file modified.

### Date: June 24, 2026 | Status: Sprint NUD-1 (Daily Decision-Logging Nudge — full send pipeline) — Retention habit-building feature: daily email + push to log one decision. Zero new env vars. Confirmed ⚑ post-95+ email/push audit priority still flagged (see §5). **External changes detected this session (diff of uploaded zip vs prior session):** `lib/graph-engine.ts` (`fetchGraphSynthesisContext` + `EMPTY_GRAPH_SYNTHESIS_CONTEXT` added — Sprint G4), `app/api/persona/route.ts` (G4 synthesis injection as Layer 3.5), `app/record/[id]/page.tsx` (graph edge count breadcrumb) — all G4, confirmed in diff, zero overlap with NUD-1. **NUD-1 shipped — 5 new files:** **(1) `lib/nudge-copy.ts`** — 30 `NudgeVariant` entries typed across 7 themes (A: daily_decision ×5, B: judgment_record ×5, C: blind_spots ×5, D: confidence ×4, E: contradictions ×4, F: long_term ×4, G: exec_lens ×3). All limits validated: push bodies ≤90 chars, email subjects ≤60 chars, email bodies ≤120 words. Voice brief: board-member/investor register, no AI mention, no feature promos, no motivational language. Two personalisation tokens resolved at send time, zero AI cost: `{{session_count}}` (3 variants — always available) and `{{bias_label}}` (2 variants — `blind_spots` theme only; excluded from eligible pool if user has no `bias_library` rows). Three exported helpers: `selectNudgeVariant(userId, now, hasBiasLabel)` — deterministic via `(dayOfYear + hash(userId)) % eligiblePool` so different users get different variants on the same day; `resolveVariantTokens(variant, sessionCount, biasLabel)` — token substitution; `toInlineBiasLabel(rawLabel)` — converts `BIAS_PARAMETERS.label` to lowercase inline form ("FOMO / Manufactured Urgency" → "manufactured urgency"). Copy review deferred to 20-user corpus milestone via `variant_index` analytics in `daily_nudge_log` (TD-nudge-1). **(2) `lib/nudge-token.ts`** — HMAC-SHA256(userId, CRON_SECRET) signed unsubscribe tokens. `generateUnsubToken(userId)` → `{userId}.{hmac}`; `verifyUnsubToken(token)` → userId | null (timing-safe compare via Node.js `timingSafeEqual`). Reuses CRON_SECRET — no new env var. Tokens break if CRON_SECRET rotates (see KDD 207). **(3) `supabase/sprint_daily_nudge.sql`** — two changes: `ALTER TABLE user_preferences ADD COLUMN IF NOT EXISTS daily_nudge_opted_out boolean NOT NULL DEFAULT false`; new `daily_nudge_log` table (`id`, `user_id` FK → auth.users ON DELETE CASCADE, `sent_at`, `variant_index smallint`), index on `(user_id, sent_at DESC)`, RLS enabled. Safe to re-run (IF NOT EXISTS throughout). **(4) `app/api/cron/daily-nudge/route.ts`** — POST, Bearer CRON_SECRET auth (same pattern as all cron routes). Targeting: authenticated (`user_id IS NOT NULL`), ≥1 session ever, last session within 180 days, no session in last 24h (logged today → skip), no nudge in last 22h (dedup window absorbs cron drift), `daily_nudge_opted_out IS NOT TRUE`. Per-user loop: email + session count via `Promise.all` (auth.admin.getUserById + sessions count); then bias query (requires email — `bias_library` is keyed by `user_email`, same pattern as `mirror-insight-email` cron); `selectNudgeVariant`; `resolveVariantTokens`; email via Resend REST (matching `reanalyze-email` pattern); push non-blocking via `sendPushToUser()`; insert to `daily_nudge_log`. HTML template: warm off-white `#f5f4f0` base, gold CTA `#c9a84c`, unsubscribe link in footer. **Resend headroom note:** free tier 3,000/mo ≈ 100/day — flag if active eligible users approach 80+. Schedule: `0 4 * * *` (04:00 UTC = 9:30 IST / 8:00 GST). **(5) `app/api/nudge/unsubscribe/route.ts`** — GET, no auth (token IS the credential). Verifies HMAC-SHA256 via `verifyUnsubToken`, upserts `daily_nudge_opted_out = true` in `user_preferences` (safe on missing rows — creates row if needed), returns branded HTML confirmation ("You're unsubscribed. Your record is still there."). Invalid token → 400 + error page (no data leaked). Idempotent. **Deployment sequence:** (1) Supabase SQL Editor → paste `sprint_daily_nudge.sql` → Run; (2) push 4 code files to Railway (`nudge-copy.ts`, `nudge-token.ts`, `app/api/cron/daily-nudge/route.ts`, `app/api/nudge/unsubscribe/route.ts`); (3) cron-job.org: new job, URL `https://app.quorumvault.org/api/cron/daily-nudge`, method POST, header `Authorization: Bearer <CRON_SECRET>`, schedule `0 4 * * *`. 5 files new, 0 schema changes post-SQL-run, 0 new Railway env vars. KDDs 205–207 added. TD-nudge-1 logged.

### Date: June 23, 2026 | Status: Decision Graph complete (Sprints G1–G4 + G3 CSP fix + record-page breadcrumb) — all 4 planned graph sprints shipped and verified in production. **Sprint G1 (schema + materialization):** new `graph_edges` table (`supabase/graph_sprint1.sql`) with 5 edge types, `dimension_breakdown` jsonb, encrypted `explanation_text`, `dismissed_at`, 4 indexes, service-role RLS. `lib/graph-engine.ts` — materialization engine with `upsertStructuralEdge` (live path from `/api/structural-match`), 4 backfill functions (`backfillStructuralEdges`, `backfillContradictionEdges`, `backfillSharedBiasEdges`, `backfillSharedDecisionTypeEdges`), `runFullBackfill` orchestrator, `decryptGraphEdge`. `app/api/graph/backfill/route.ts` — `INTERNAL_API_SECRET`-gated admin backfill endpoint. `app/api/structural-match/route.ts` patched to write `structural_similarity` edges live for authenticated users clearing `MATCH_THRESHOLD`. Pair canonicalisation (UUID lexicographic, `session_id_a < session_id_b`) enforced before every upsert; the unique constraint on `(session_id_a, session_id_b, edge_type)` covers exactly one row per pair per type. **Sprint G2 (query API + curation):** `GET /api/mirror/graph` — nodes + edges, Mirror-gated, corpus-gated (`MIN_GRAPH_SESSIONS=20`, `MIN_GRAPH_EDGES=3`, Advisory bypasses via `ADVISORY_BYPASSES_THRESHOLDS`), `?types=` filter, `?include_dismissed=true`. `POST /api/mirror/graph/edges` — user_asserted edge creation (ownership of both sessions verified server-side, 400-char limit, `explanation_text` encrypted, idempotent upsert). `PATCH /api/mirror/graph/edges/[id]/dismiss` — scoped to `user_id`, idempotent, Next.js 15 `Promise<{ id: string }>` params pattern (caught by `next build`, not `tsc --noEmit` — see KDD 201). **Sprint G3 (d3-force Mirror UI):** `components/DecisionGraph.tsx` — loads d3 v7.9 via `import('d3')` (dynamic import, code-split, no CDN). Node click → `/record/[id]`. Edge click → tooltip with `top_matching_dims` / explanation / bias parameter labels / dismiss button. `+ Add connection` form → `POST /api/mirror/graph/edges`. Computed edges (solid) vs `user_asserted` (dashed gold) visually distinct per KDD design. Corpus gate with progress bar. `MirrorNav.tsx` — `{ key: 'graph', label: 'Graph' }` pill added. `app/mirror/page.tsx` — `DecisionGraph` imported and rendered in `<div id="msec-graph">` between Benchmark and subscription management. **G3 CSP incident (same day):** initial delivery used a CDN `<script>` tag for d3 which was blocked by Railway CSP (`script-src 'self' 'unsafe-inline' https://checkout.razorpay.com`). API was working (graph returned 200 with 37.5 kB) but `loadD3()` rejected → `setError(true)` → "Unable to load graph." Fixed by replacing CDN loader with `import('d3')` dynamic import. Added `d3: ^7.9.0` and `@types/d3: ^7.4.3` to `package.json`. No CSP change needed. Production screenshot confirmed working: 49 decisions · 48 connections. **Sprint G4 (synthesis integration):** `fetchGraphSynthesisContext` and `EMPTY_GRAPH_SYNTHESIS_CONTEXT` added to `lib/graph-engine.ts`. Queries current session's edges (structural_similarity, contradiction, shared_bias_trigger) with a single 800ms retry to handle the structural-match race condition. Also queries `shared_decision_type` count (doesn't depend on current-session edges). When `connectedCount >= 2`, injects a Pattern Analyst elevation directive. Injected as Layer 3.5 in `app/api/persona/route.ts` — after bias block (Layer 3), before council-weighting relevance directive (Layer 4). Non-fatal: empty string = complete no-op. 3 files changed: `lib/graph-engine.ts`, `app/api/persona/route.ts`. **Record-page breadcrumb (same session):** `app/record/[id]/page.tsx` — graph edge count added to the parallel `Promise.all` at page load. `graphConnectionCount` shown as "Connected to N decisions in your graph →" badge below `parentLink`/`childLink` breadcrumbs. Links to `/mirror#msec-graph`. Only shown to authenticated Mirror users with `graphConnectionCount > 0`. For non-chain-root sessions, Mirror access is checked here if not already set by the `isChainRoot` block. 1 file changed. No schema changes, no new env vars (beyond `MIN_GRAPH_SESSIONS=20` and `MIN_GRAPH_EDGES=3` which have in-code defaults). KDDs 201–202 added. **Stakeholder/people graph** is explicitly logged as a known future initiative — deferred pending (a) corpus scale and (b) separate privacy/data-capture review for named-entity data. See KDD 202.
### Date: June 23, 2026 | Status: Sprint UX-1 (UX fixes: theme persistence, breadcrumb colour, What Keeps Coming Up CTA, persona card height evaluation) — Four issues evaluated; three shipped, one explicitly rejected. **(1) Theme persistence (`components/ThemeToggle.tsx` — full rewrite):** root cause was `useState('dark')` hardcoded default — the toggle button always initialised to dark on every page load/SPA navigation and corrected itself in a `useEffect`, causing a flash of the wrong label on every navigation. Fix: state initialised as `null` (not `'dark'`); new `getPersistedTheme()` helper reads `localStorage.getItem('quorum_theme')` synchronously before render, falls back to `document.documentElement.getAttribute('data-theme')` (whatever the existing anti-flash inline script in `layout.tsx` already stamped). Component renders `null` until theme is known — page background is already correct from the inline script; only the toggle button label waits one tick. On every `useEffect` mount, preference is re-read from `localStorage` and DOM attribute re-synced — forward/back navigations all pick up the stored preference immediately. 1 file changed. KDD 201. **(2) Breadcrumb colour re-fix (`app/record/[id]/page.tsx`):** "← Revisiting a decision from [date]" and "Revisited on [date] →" breadcrumbs (added RET-5 Sprint 1) were using `var(--gold-dim)` — resolves to `#5a3f1a` in dark mode (near-invisible). Prior RET-5 Fixes entry scoped the same fix but the correct file version was not confirmed landed in Ken's deployed code. Fix applied directly here: both breadcrumb `color` values `var(--gold-dim)` → `var(--gold)`. 1 file changed, overlaps with RET-5 fix batch file. **(3) "What Keeps Coming Up" Mirror CTA (`components/RecurringConditionCard.tsx`, `app/page.tsx`):** initial approach — add decision evidence toggle matching `PatternSurfaceCard` format exactly (toggle button, `→` linked decision rows, "Show N more", `DECISIONS WHERE THIS FIRED" header). Implemented, deployed, then correctly identified a data integrity problem: `top_dimensions` from `/api/mirror/patterns` returns `high_count` per dimension but not the session IDs that produced it. Using the N most-recent sessions as evidence would show misleading data — random recent decisions, not the ones where the dimension actually fired. KDD 202. Decision: remove evidence section; replace with single "See all patterns in detail → Mirror" CTA linking to `/mirror#msec-patterns` (PatternStore, which carries proper `session_ids` per pattern and shows evidence correctly). `components/RecurringConditionCard.tsx` — all evidence state/refs/toggle logic removed; stateless again; CTA row added (`See all patterns in detail` + `Mirror →` gold right, hover border lift). `app/page.tsx` — `sessions={sessions}` prop removed from `<RecurringConditionCard>` call. 2 files changed, 0 schema changes, 0 new env vars. **(4) Persona card equal height (evaluated, rejected):** equal height without max-height/scroll not achievable cleanly when card body is AI-generated and varies 3–10x in length. Alternative evaluated: `margin-top: auto` on card content area anchors footer row to card bottom consistently without enforcing equal heights. Ken elected to skip entirely for now — no code shipped.
...
> **Not reflected below (added this pass, not fully backfilled into this section — see TSD §12 for the complete schema):** `synthesis_versions` (P1, July 16–17 — see dated entries above), and the entire Institutional Mode schema (`institutions`, `institution_memberships`, `consent_audit_log`, `cohorts`, `cohort_memberships`, `user_institution_preference`, `seen_unlock_notices`, plus three aggregate views and one restricted DB role — July 7–14). This section's table-by-table encrypted-columns breakdown below predates all of that and was left as-is rather than fully rewritten in this pass; none of the new tables carry encrypted free-text columns of the kind this section tracks, with one exception worth noting directly here since it's easy to miss in the TSD's denser §12 treatment: `graph_edges.explanation_text`-style free text does NOT exist on any institutional table, but `bias_library.activation_contexts` (pre-existing, encrypted) is deliberately excluded from the Sprint 6 bias-parameter-benchmark view specifically because it can carry decision-specific free text — worth keeping in mind if a future institutional view is ever built against a table with encrypted columns.
...
### user_preferences ← Sprint 21 + Sprint M1 + Sprint NUD-1
...
`daily_nudge_opted_out boolean NOT NULL DEFAULT false` column added (Sprint NUD-1 migration — `supabase/sprint_daily_nudge.sql`). Set `true` via one-click unsubscribe in daily nudge email footer (`/api/nudge/unsubscribe?token=<hmac>`). Read by `app/api/cron/daily-nudge/route.ts` to exclude opted-out users from send pool. Use `ALTER TABLE user_preferences ADD COLUMN IF NOT EXISTS daily_nudge_opted_out boolean NOT NULL DEFAULT false;` if running migration manually.
...
```
lib/
  nudge-copy.ts            — ✅ Sprint NUD-1 NEW — 30-variant daily nudge copy bank. NUDGE_VARIANTS[]: typed push + email pairs across 7 themes (daily_decision, judgment_record, blind_spots, confidence, contradictions, long_term, exec_lens). Board-member/investor voice throughout; no AI mention; no feature promos. Two personalisation tokens: {{session_count}} (3 variants), {{bias_label}} (2 variants — excluded from pool if user has no bias data). Exports: selectNudgeVariant(userId, now, hasBiasLabel) — deterministic (dayOfYear + hash(userId)) % eligible; resolveVariantTokens(variant, count, biasLabel); toInlineBiasLabel(rawLabel). Copy review TD-nudge-1 at 20-user corpus.
  nudge-token.ts           — ✅ Sprint NUD-1 NEW — HMAC-SHA256 signed unsubscribe tokens for daily nudge emails. generateUnsubToken(userId): format {userId}.{hex-hmac}, key = CRON_SECRET. verifyUnsubToken(token): timing-safe compare via Node.js timingSafeEqual; returns userId | null. No new env var — reuses CRON_SECRET. Tokens invalidate if CRON_SECRET rotates (KDD 207).
...
app/api/
  cron/daily-nudge/route.ts    — ✅ Sprint NUD-1 NEW — POST, Bearer CRON_SECRET. Targets active authenticated users: last session ≤180d, no session today (24h), not nudged in 22h, daily_nudge_opted_out IS NOT TRUE. Per-user: email + session count (Promise.all); bias via bias_library.user_email; selectNudgeVariant; resolveVariantTokens; Resend email; sendPushToUser() non-blocking; daily_nudge_log insert. Schedule: 0 4 * * * on cron-job.org (04:00 UTC = 9:30 IST). Resend headroom: flag at 80+ active users (free tier ≈100/day).
  nudge/unsubscribe/route.ts   — ✅ Sprint NUD-1 NEW — GET, no auth (HMAC token = credential). verifyUnsubToken() → upsert daily_nudge_opted_out = true → branded HTML confirmation. Invalid token → 400 error page. Idempotent. Links generated by daily-nudge cron via generateUnsubToken() from lib/nudge-token.ts.
...
**KDD 201 — `ThemeToggle` must initialise state as `null`, not `'dark'`, and read `localStorage` synchronously in `useEffect` to prevent the toggle button showing the wrong mode label on every page load and SPA navigation (Sprint UX-1, June 23, 2026).** The original implementation had `useState('dark')` as the default — React always hydrates this as dark mode regardless of the user's stored preference, then a `useEffect` reads `document.documentElement.getAttribute('data-theme')` and corrects it. The correction fires after paint, producing a flash of the wrong button label (`Light` when the page is actually already in light mode) on every page navigation. The correct pattern: initialise state as `null`, read `localStorage.getItem('quorum_theme')` directly in `useEffect` (not the DOM attribute — DOM attribute is set by the anti-flash inline script and is correct, but `localStorage` is the canonical source), set both state and the DOM attribute from the persisted value, render `null` (not a default button) until the first `useEffect` completes. The anti-flash inline script (`layout.tsx`) handles page-background correctness before paint; `ThemeToggle` only needs to wait one tick for the button label. This pattern ensures the button always shows the correct mode after every navigation without flash. The existing `localStorage.setItem('quorum_theme', next)` on toggle is unchanged and correct.

**KDD 202 — `top_dimensions` from `/api/mirror/patterns` carries `high_count` (count of sessions where the dimension scored high) but not the session IDs that produced that score; using the homepage `sessions` array as evidence for `RecurringConditionCard` would surface misleading data (Sprint UX-1, June 23, 2026).** The `PatternSurfaceCard` component works correctly because it consumes `pattern.session_ids` — an explicit list of session IDs where each specific named pattern (from the rule engine) fired, returned per-pattern by the patterns API. `top_dimensions` is a separate, aggregated signal from `sessions_ontology` scoring: it tells you *how many* sessions scored high on a structural dimension, but not *which* ones. Slicing the N most-recent sessions from the homepage `sessions` array to fill that count produces a list of random recent decisions — not the decisions where the dimension actually fired — which is actively misleading as evidence. The correct fix for a future sprint: add `session_ids: string[]` to each entry in the `top_dimensions` response (populated in `app/api/mirror/patterns/route.ts` by querying `sessions_ontology` rows where the dimension scored above threshold). Until that field exists, do not render any session-level evidence on `RecurringConditionCard`. The current implementation replaces the evidence section with a CTA to `/mirror#msec-patterns` (PatternStore), which does have proper session-ID attribution and can show evidence correctly.

**KDD 203 — The Decision Graph (Sprints G1–G4) is a decision-to-decision graph only; stakeholder/people nodes are explicitly deferred (June 23, 2026).** The graph architecture currently materialises 5 edge types: `structural_similarity` (cosine similarity across 14 ontology dims), `contradiction` (from `contradiction_log`), `shared_bias_trigger` (from `bias_library.session_ids`), `shared_decision_type` (from `sessions_ontology.decision_type_primary`), and `user_asserted` (user-authored one-sentence causal link). A stakeholder/relationship graph (nodes = named people or organisations; edges = influence, counterparty, relationship) was explicitly evaluated and deferred for the following reasons: (a) Quorum currently stores no named-entity data — there are no people, counterparties, or organisations as first-class DB records; adding them requires new data-capture UI, new schema tables, and a separate PII/privacy review given the HNI clientele; (b) this is a materially different product surface from the decision-to-decision graph (it answers "who influences whom", not "how decisions connect"), closer to Kumu/Palantir than to the CBR/GraphRAG architecture this graph was designed around. When the stakeholder graph is scoped as its own initiative, it should be designed as a distinct module, not bolted onto `graph_edges`. The `user_asserted` edge type is the only category-6 (manual linking) element in the current graph — intentionally narrow, for causal/narrative links the system cannot infer, with a 400-char limit and visual distinction from computed edges. Do not expand `user_asserted` semantics to cover stakeholder relationships until the stakeholder initiative is formally scoped.

**KDD 204 — `graph_edges` is written live only for `structural_similarity` edges; the three other computed types (`contradiction`, `shared_bias_trigger`, `shared_decision_type`) are backfill-only until a cron is added (June 23, 2026).** The live write path (in `app/api/structural-match/route.ts`) fires `upsertStructuralEdge` for qualifying pairs as each new session is scored. The other three types depend on data that is either batch-updated (`bias_library.session_ids`, updated by the bias trigger engine which runs on demand) or requires a cross-session query (`contradiction_log`, `sessions_ontology` grouping by `decision_type_primary`) — neither has a natural single-session trigger point. For now: run `POST /api/graph/backfill` (INTERNAL_API_SECRET-gated) manually after any significant new session batch, or set up a nightly Railway cron calling that endpoint for each active `user_id`. The backfill is fully idempotent — all upserts use `onConflict: 'session_id_a,session_id_b,edge_type'`, so re-running it is safe. A future sprint can add a lightweight cron (`app/api/cron/graph-refresh/route.ts`) following the same pattern as the existing `reanalyze-email` cron.



 (`user_id` > `user_email` > `device_id`) is applied consistently across all three RET-sprint surfaces, and `device_id`-only users are intentionally excluded from `echo-hint` (Sprint RET-1/RET-3, June 20, 2026).** `BiasNoteCard`'s server-side query in `record/[id]/page.tsx` and `echo-hint/route.ts` both resolve identity using the same precedence order already established by `/api/bias-score` for writes. `BiasNoteCard` supports all three tiers including `device_id`-only (anonymous) users — bias detection already works pre-email per existing `bias_library` design, so there is no reason to exclude anonymous users from session-1 bias feedback. `echo-hint`, by contrast, requires `user_id` or `user_email` and explicitly returns `{ available: false, reason: 'no_identity' }` for device-only sessions — this is a deliberate asymmetry, not an oversight: it gives `EmailCaptureCard` a second, concrete, structural-comparison-specific reason to convert an anonymous user ("link your email to start seeing pattern connections"), reinforcing rather than competing with that card's purpose. Do not "fix" this asymmetry by adding `device_id` support to `echo-hint` without first confirming `EmailCaptureCard`'s conversion data doesn't depend on it.

**KDD 205 — Daily nudge copy is a static bank, not runtime-generated; personalisation tokens are the only per-user variation (Sprint NUD-1, June 24, 2026).** Two options were evaluated: (A) generate nudge copy per-user via Claude at send time — per-user personalisation, but adds latency, cost, and tone-control risk to every cron send; (B) static 30-variant bank selected deterministically, with two lightweight tokens (`{{session_count}}`, `{{bias_label}}`) resolved from existing DB data at send time. Option B chosen for three reasons: (1) tone is locked and reviewable before a single email goes out — board-member/investor register cannot drift at scale; (2) zero AI call per send, no latency or cost; (3) the 30-entry pool gives a 30-day cycle before a user repeats a variant. The personalization tokens add real signal — `{{bias_label}}` names the user's confirmed recurring tendency, making the nudge feel like it knows the user without any generative overhead. If variant quality is later validated against return-to-app data via `variant_index` analytics (TD-nudge-1), the mix can be retuned by editing the static bank, not by adding an AI call.

**KDD 206 — Daily nudge targeting uses three independent filters in sequence; the 22h cooldown window is shorter than 24h by design (Sprint NUD-1, June 24, 2026).** The three filters are: (1) last session within 180 days (`ACTIVE_WINDOW_DAYS`) — cold users risk spam flags and should not receive daily outbound; (2) no session in the last 24h (`LOGGED_TODAY_HOURS`) — users who already logged today need no nudge; (3) no nudge sent in the last 22h (`NUDGE_COOLDOWN_HOURS`) — the 22h window, not 24h, is intentional: a cron scheduled at 04:00 UTC drifts slightly across clock changes and scheduling jitter. A 24h window would eventually skip a day for some users (if cron fires at 04:01 on one day and 03:59 on the next, the 24h window between them would exclude the second send). 22h absorbs this drift cleanly. The effective real-world interval for a daily recipient is 22–26h, which is perceptually daily. `ACTIVE_WINDOW_DAYS`, `LOGGED_TODAY_HOURS`, and `NUDGE_COOLDOWN_HOURS` are constants in the route file — adjust there, not in Railway env vars (these are product-logic constants, not deployment-environment parameters).

**KDD 207 — Unsubscribe tokens reuse CRON_SECRET as the HMAC key; tokens invalidate if CRON_SECRET rotates (Sprint NUD-1, June 24, 2026).** Two approaches were evaluated: (A) new dedicated `NUDGE_UNSUB_SECRET` env var — cleaner separation, tokens survive CRON_SECRET rotation; (B) reuse existing `CRON_SECRET` — no new env var, smaller Railway footprint, acceptable risk at current scale. Option B chosen: at current user count (single-digit to low double-digit active users), a CRON_SECRET rotation is a manual event, not a scheduled rotation — if it ever happens, affected users can simply reply to any Quorum email and the opt-out can be manually applied in Supabase (`UPDATE user_preferences SET daily_nudge_opted_out = true WHERE user_id = ...`). The error HTML on an invalid token explicitly instructs this fallback. If CRON_SECRET rotation becomes routine (e.g. staff turnover with env-var access, automated rotation policy), introduce a dedicated `NUDGE_UNSUB_SECRET` and re-sign all future unsubscribe URLs. The verification uses Node.js `timingSafeEqual` to prevent timing oracle attacks regardless of key choice.
...
| **Mirror** | ₹3,999/mo · ₹39,999/yr | Self-serve — live via Razorpay (Sprint CX-PAY, June 16, 2026; corrected here — this row previously said "pending," stale as of that sprint) | Unlimited |
...
| Sprint | What shipped |
|---|---|
| **Sprint NUD-1 — Daily Decision-Logging Nudge (June 24, 2026)** | **5 new files. `lib/nudge-copy.ts` — 30-variant copy bank, 7 themes, board-member voice, ≤90-char push bodies, ≤60-char subjects, ≤120-word email bodies, validated. {{session_count}} + {{bias_label}} tokens resolved at send time, zero AI cost. Helpers: selectNudgeVariant (deterministic rotation), resolveVariantTokens, toInlineBiasLabel. `lib/nudge-token.ts` — HMAC-SHA256 signed unsubscribe tokens reusing CRON_SECRET. `supabase/sprint_daily_nudge.sql` — daily_nudge_log table (id, user_id, sent_at, variant_index) + daily_nudge_opted_out column on user_preferences. `app/api/cron/daily-nudge/route.ts` — targeting: ≤180d active, no session today, 22h cooldown, not opted-out; per-user variant selection + email + push (non-blocking) + log. `app/api/nudge/unsubscribe/route.ts` — GET, token-verified opt-out, branded HTML confirmation. Schedule: 0 4 * * * (cron-job.org). 0 new env vars. KDDs 205–207. TD-nudge-1 logged. — ✅ Built June 24, 2026 — deployment pending (SQL + Railway + cron-job.org)** |
| **Sprint UX-1 — Theme persistence, breadcrumb colour, What Keeps Coming Up CTA (June 23, 2026)** | **`components/ThemeToggle.tsx` — full rewrite: `useState('dark')` default removed; state initialised as `null`; `getPersistedTheme()` reads `localStorage` synchronously; component renders `null` until theme known (eliminates button-label flash on every page load). `app/record/[id]/page.tsx` — revisit breadcrumb colour `var(--gold-dim)` → `var(--gold)` (re-fix: prior RET-5 patch not confirmed landed). `components/RecurringConditionCard.tsx` — evidence section removed (KDD 202: `top_dimensions` carries `high_count` but not session IDs — evidence would be misleading); replaced with Mirror CTA linking to `/mirror#msec-patterns`. `app/page.tsx` — `sessions` prop removed from `<RecurringConditionCard>` call. 4 files changed, 0 new files, 0 schema changes, 0 new env vars. KDDs 201–202. — ✅ Implemented June 23, 2026** |
...
**TD-nudge-1 (copy review at 20-user corpus) —** At 20+ active users, run: `SELECT variant_index, COUNT(*) as sends FROM daily_nudge_log GROUP BY variant_index ORDER BY variant_index;` Cross-reference with same-day return-to-app events to identify which themes drive the highest re-engagement rate. Adjust copy mix if any theme underperforms (shift theme distribution in `NUDGE_VARIANTS`, keeping 30 total). Defer to corpus milestone — no earlier signal is reliable. Logged Sprint NUD-1 (June 24, 2026).
...
- **Tiered AI Routing / Product Tiers / Sprint 12 Google Sign-In / Session Sharing (July 20–28, 2026) — largest undocumented batch found this pass; see TSD §2.21/§2.22/§6.3/§9.2–§9.5 and the dated entries at the top of this document's log:**
  - **Zero paying users on Elite or Private as of the tier-columns migration** (per the migration's own comment) — none of the tiered-routing/tier-resolution code path has been exercised against real billing data yet. Worth a deliberate smoke test (a real or test-mode Elite signup, confirming `product_tier` lands as `'elite'` and routes correctly once `TIERED_ROUTING_ENABLED` is flipped on) before the next real signup lands on any tier.
  - `TIERED_ROUTING_ENABLED` is off by default and not currently set in Railway per the codebase default — confirm intended state before assuming any account is being tier-routed today. All of §9.2–§9.4's mechanism is real and wired, but functionally dormant until this flag flips.
  - `lib/tier-context.ts`'s per-iteration override is **not yet wired into `daily-nudge` or `reanalyze-email`** — flagged in the file's own doc comment as an open follow-up. Needed before those two cron routes can participate in tiered routing at all; each route's actual AI-call chain needs tracing first.
  - Private tier's self-hosted Qwen/Mistral endpoints (env vars `QWEN_SELFHOSTED_*`) **do not exist yet** — separate GPU-provisioning/deploy-tooling track. The code path fails loudly (by design) rather than silently falling back to a different tier's model — don't mistake a loud failure here for a bug.
  - Mistral-family prompt-quality extension (evidence discipline + synthesis/persona depth push, TSD §2.22) is scoped to the six personas + synthesis only — Examiner and the structured-extraction calls (`lib/ontology-tagger.ts`, `lib/bias-scorer.ts`) route to Mistral under the same tiers and share the same hallucination exposure in principle, but weren't covered by the reference comparison transcripts. Extending the fix to those call sites is the explicit next piece of this thread, not yet started.
  - `lib/synthesis-highlights.ts` (Session Sharing's tag parser, TSD §2.21) is a fourth independent copy of the `<verdict>`/`<conditions>`/`<key_question>`/`<action_plan>` tag-stripping logic and is **not covered by the `tests/tag-wiring-guardrail.test.ts` guardrail** added the same week (that suite's `SINK_FILES` list still only names the original five). Worth deciding whether to extend the guardrail to a sixth sink or treat the share feature's narrower tag needs as out of scope for that test on purpose.
  - Confirm the actual Razorpay `plan_id`s referenced by `RAZORPAY_MONTHLY_PLAN_ID`/`RAZORPAY_ANNUAL_PLAN_ID` reflect the ₹2,999/₹29,999 pricing found in the current front-end copy — this pass corrected the *documentation* to match the code, but didn't have access to the live Razorpay dashboard to confirm the actual configured plan amounts agree.
- **Institutional Mode (July 7–14, 2026) — undocumented until this pass; no session ever scoped or reviewed it. Reconciled against `TECH_DEBT.md` (Kunal's own tracker) this pass — two items below are now sourced from there, not from code-reading, and are more authoritative than the rest of this list as a result. Before pursuing any pilot org, work through these in order:**
  - **No session has actually reviewed this feature end-to-end.** Everything in the dated entries above and TSD §12 is reconstructed from code/schema/mtimes, not from a design conversation. Worth a dedicated session just to confirm the architecture matches what you actually intended, before the first pilot institution goes live.
  - **Naming collision worth fixing before it causes confusion:** this document's "Institutional Sprint 6" (July 12, bias-parameter-view work — named after the SQL migration filename) is **not** the "Sprint 6 (Hardening, Edge Cases, Rollout)" your tracker refers to as the sprint where the two items below get swept. That hardening sprint hasn't happened. Wherever "Sprint 6" appears in this document or the TSD referring to the bias-parameter work, it does not mean hardening is done.
  - `NEXT_PUBLIC_INSTITUTIONAL_MODE_ENABLED` — confirm current Railway state. If it's already on, confirm you know that and it's deliberate; if off, nothing below is reachable regardless.
  - **Billing gap, worth resolving before any pilot:** there is no in-product billing for this tier at all — no Razorpay plan, no seat billing. Provisioning is entirely manual via `ADMIN_CODE` + a distributed unlock code. Separately: an institution member gets Cohort Insights and (if their institution clears K-floor) the aggregate benchmark **regardless of whether they personally hold a paid Mirror subscription** — `lib/mirror-access.ts` has zero awareness of institutions. Confirm this is the intended commercial shape (the institution's arrangement is what's being paid for) rather than an oversight — it means an institutional deal currently has no built-in path to also convert members into individual paying subscribers.
  - **[Tracker Tech Debt #1, Institutional Sprint 2]** `POST /api/institutions/consent` writes the membership update and the audit-log insert as two separate calls, not one transaction — an audit-insert failure right after a successful update leaves the consent change correctly applied but silently un-logged (fails loud in server logs via `console.error`, not silently in the data). Tracker's fix: wrap both in one Postgres function, call via `supabase.rpc('toggle_consent', ...)`. Scoped by the tracker for the hardening sprint above, or sooner if a real audit-log gap actually surfaces in practice.
  - **[Tracker Tech Debt #2, Institutional Sprint 3]** Roster and cohort-insights email resolution is N+1 (`auth.admin.getUserById()` per member/peer) — tracker's fix is a Postgres function joining directly to `auth.users`, called once via `.rpc(...)` instead of N calls, applied to both `.../admin/roster/route.ts` and `lib/cohort-insights.ts` together. **Tracker's own priority note: revisit before onboarding any institution near the top of the size tiers in your institutional plan's Section 1.1** — sooner than the hardening sprint if a real customer gets there first. (That plan document isn't something this reconstruction has visibility into — if you want future passes of this doc to reference its size tiers directly rather than gesturing at "Section 1.1," share it.)
  - `aggregate_reader` restricted DB role (Sprint 4, July 9) is defined but **nothing in the app uses it** — every institutional route still queries via full-access `createServiceClient()`. Not on the tracker as of the file shared this pass — worth adding to it, and prioritizing wiring this before pilot data volume grows.
  - `consent_aggregate_backfill` is stored and settable but no aggregate view branches on it — either implement the distinction or drop the column before a real user sees a toggle that does nothing.
  - No institution-admin-facing self-deactivation path — only platform-admin (`ADMIN_CODE`) can deactivate. Confirm intentional.
  - `k_floor_default()` (SQL) and `DEFAULT_K_FLOOR` (`lib/k-floor.ts`) are two independently-maintained copies of the same constant (`20`) — no shared source of truth; a future change to one without the other silently desyncs the privacy floor between the views and the application layer.
  - Sprint CAL's calibration-zone boost and the institutional synthesis-context injection (both additive layers in the same synthesis prompt, added in unrelated sprints) have never been tested together for a user who is both — see the July 17 dated entry above.
  - Account deletion (`DELETE /api/account`) does not clean up `institution_memberships` or `cohort_memberships` — orphaned rows possible after a deleted account; low urgency at pilot scale (aggregate views are count/average-based, a phantom member slightly skews a denominator) but worth a cleanup pass eventually.
- **P1 (Worth Confirming + What Changed) and Examiner Resolvability Check (July 16–17, 2026) — shipped, undocumented until this pass:**
  - Neither `lib/worth-confirming.ts` nor `lib/synthesis-diff.ts` carries a sprint codename in its own header comment — "Sprint 1 follow-on" / "P1" is reconstructed from a single inline comment elsewhere. Worth confirming these names against whatever you actually called this work, so future sessions reference it consistently.
  - `components/CouncilWeightingStrip.tsx` was touched in the same batch as Worth Confirming/synthesis-diff but with no narrating comment — the specific change wasn't confirmed against a diff, only inferred. Worth a quick look to confirm it's just reading the new headers and nothing more.
  - `lib/examiner-resolvability-check.ts`'s own comment references "recommendation #3" from an audit whose other recommendations aren't identifiable anywhere in this codebase — worth checking your own notes for whether #1 and #2 are still open.

- **Sprint NUD-1 (Daily Nudge) — built June 24, 2026. Deployment steps (all three required before cron fires):**
  - Step 1 — SQL: Supabase Dashboard → SQL Editor → paste `supabase/sprint_daily_nudge.sql` → Run. Confirm: `daily_nudge_log` table exists + `user_preferences.daily_nudge_opted_out` column exists.
  - Step 2 — Deploy: `git add lib/nudge-copy.ts lib/nudge-token.ts app/api/cron/daily-nudge/route.ts app/api/nudge/unsubscribe/route.ts && git commit -m "feat: daily decision-logging nudge (NUD-1)" && git push` → Railway autodeploys. Confirm green.
  - Step 3 — cron-job.org: Dashboard → Create cronjob → URL `https://app.quorumvault.org/api/cron/daily-nudge`, method POST, header `Authorization: Bearer <CRON_SECRET>`, schedule `0 4 * * *` (04:00 UTC). Use "Execute now" to smoke-test (expect `{ok:true,sent:0,...}` or a real send if you haven't logged today).
  - Smoke test (before cron fires): `curl -s -X POST https://app.quorumvault.org/api/cron/daily-nudge -H "Authorization: Bearer <CRON_SECRET>" | jq`
  - Force a test send to yourself: temporarily comment out the 24h `loggedTodayCutoff` filter in route, deploy, hit endpoint, revert.
  - No new Railway env vars required.
  - TD-nudge-1: review `variant_index` analytics at 20-user corpus milestone.
- **Decision Graph (Sprints G1–G4) — complete as of June 23, 2026. Next steps:**
  - Run `supabase/graph_sprint1.sql` in Supabase SQL Editor if not yet executed — this is the only deployment step not handled by Railway auto-deploy.
  - Call `POST /api/graph/backfill` (`Authorization: Bearer <INTERNAL_API_SECRET>`, body `{ "user_id": "<uuid>" }`) once per existing user to materialise edges from the existing corpus. Idempotent — safe to re-run.
  - `structural_similarity` edges now write live on every new scored session. The other three computed types (`contradiction`, `shared_bias_trigger`, `shared_decision_type`) are backfill-only — run `POST /api/graph/backfill` after any significant session batch, or add a nightly cron (see KDD 204).
  - Two new optional env vars: `MIN_GRAPH_SESSIONS` (default `20`) and `MIN_GRAPH_EDGES` (default `3`). Both have in-code defaults — only needed in Railway if you want to lower the corpus gate for early testing (e.g. `MIN_GRAPH_SESSIONS=5` during your own advisory usage).
  - `lib/supabase.ts` split (flagged in TB1, still not done): `createServiceClient` has 57 import sites — deferred until a dedicated sprint. See TB1 dated entry. Not urgent; no live bug.
  - **Graph recall breadcrumb on record page** — `app/record/[id]/page.tsx` updated to show "Connected to N decisions in your graph →" badge for Mirror users. Verified via `next build`. 1 file changed, no schema changes.
  - **Stakeholder/people graph** — explicitly deferred. See KDD 203 for the rationale and what would need to be scoped before starting.
  - **Graph cron** — `POST /api/graph/backfill` should eventually be called by a nightly Railway cron to keep contradiction/bias/decision_type edges current. Pattern identical to `reanalyze-email` cron. Not yet built. See KDD 204.
- **Sprints RET-1/2/3/4/5 — pending verification items** (unchanged from v48 — listed here for completeness, not re-litigated):
  - Levers (i) and (ii) of retention strategy fully shipped. Lever (iii) parked — scope with Ken before building.
  - `app/api/brief-access/route.ts` and `BRIEF_ACCESS_TOKEN` Railway env var — dead code since RET-1, flagged for deletion, not yet removed.
  - Confirm `EarlyEchoCard` degrades correctly for no-email sessions (count-only copy, no error).
  - Confirm `EmailCaptureCard` → `/api/auth` → magic-link round-trip from record page.
  - `BiasNoteCard` with `device_id`-only session — verify `bias_library` query resolves correctly.
- **Sprint CX-PAY — Razorpay live-mode pending** (unchanged from v48 — confirm KYC cleared, swap to live keys, watch first real transaction).
- **Other deferred items** (unchanged): DeepSeek env var verification, Engagement/PWA cron + SQL migrations, `last_mirror_viewed_at` migration, M4 `first_confirmed_at` column, M5 API batching, website copy sync.
...
RAZORPAY_MONTHLY_PLAN_ID ← Sprint CX-PAY NEW — plan_xxx from Razorpay Dashboard → Subscriptions → Plans.
                           ₹2,999/month, billing_period=monthly, billing_frequency=1. (Corrected this
                           pass — this line previously read ₹3,999; the live plan and all current
                           front-end copy — PaymentButton.tsx, FAQSection.tsx, mirror/page.tsx — agree
                           on ₹2,999. Confirm the actual Razorpay plan_id still matches before assuming
                           this was just a doc typo.)
RAZORPAY_ANNUAL_PLAN_ID  ← Sprint CX-PAY NEW — plan_xxx from Razorpay Dashboard → Subscriptions → Plans.
                           ₹29,999/year, billing_period=yearly, billing_frequency=1. (Corrected this
                           pass — see note on RAZORPAY_MONTHLY_PLAN_ID above; was ₹39,999.)
...
NEXT_PUBLIC_INSTITUTIONAL_MODE_ENABLED ← Institutional Mode NEW (July 2026) — default off. Master kill
                           switch for the entire B2B layer (§12 of the TSD). NEXT_PUBLIC_-prefixed like
                           the Watchlist flag — inlined into the client bundle at build time, so a Railway
                           restart alone will NOT pick up a change; requires a rebuild. Every institutional
                           route checks this server-side and returns a bare 404 (not an empty 200) when off.
                           Not currently set in Railway per the codebase's default — confirm current state
                           if you intend to demo or pilot this.
ADMIN_CODE               ← Reused by Institutional Mode for POST /api/admin/create-institution (platform-
                           admin-only institution creation/deactivation) — same secret used elsewhere for
                           platform-admin actions, not a new var, but newly load-bearing for this feature
                           if it wasn't already set.
TIERED_ROUTING_ENABLED   ← NEW (July 27, 2026) — true | false, default false. Master switch for the
                           Free/Elite/Private tiered AI-routing layer (TSD §9.3) sitting on top of the
                           AI_PROVIDER/ROUTING_MODE routing above. When false (the current default),
                           every var above this line behaves exactly as documented with zero change.
                           Not currently set in Railway per the codebase default — confirm before assuming
                           any account is actually being tier-routed.
ELITE_PREMIUM_MODEL      ← NEW (July 27, 2026) — Claude model used for Elite-tier "premium" role calls
                           under tiered routing. Default: claude-sonnet-4-6. Deliberately its own var,
                           not a repurposed ANTHROPIC_MODEL, so changing it can never affect legacy
                           hybrid/deepseek_only behavior.
MISTRAL_API_KEY          ← NEW (July 27, 2026) — required once TIERED_ROUTING_ENABLED=true, since Free
                           tier and Elite's "fast" role both route to Mistral Small (cloud).
MISTRAL_MODEL            ← NEW (July 27, 2026) — Mistral model override. Default: mistral-small-latest.
QWEN_SELFHOSTED_BASE_URL, QWEN_SELFHOSTED_API_KEY, QWEN_FAST_MODEL, QWEN_PREMIUM_MODEL
                         ← NEW (July 27, 2026) — Private-tier (Option A) self-hosted endpoints. Read
                           lazily, only when an actual Private/Qwen-tier call is made, so an unconfigured
                           deployment doesn't block Free/Elite. **Infra does not exist yet** (separate
                           GPU-provisioning/deploy-tooling track) — an error from this branch means the
                           infra isn't live, not that the feature is broken. See TSD §9.2.
NEXT_PUBLIC_APP_URL      ← Already in use for the magic-link redirect (§6.1); as of Sprint 12 (July 27,
                           2026) also used to build the Google OAuth redirectTo (must match Supabase's
                           allowlist) and, as of the same-week Session Sharing feature, to build public
                           share:// URLs (falls back to https://app.quorumvault.org if unset).
```
