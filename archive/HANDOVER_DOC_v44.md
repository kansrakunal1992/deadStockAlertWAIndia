...
| After RET-3 (Item A + Item C + copy fix; RET-2 folded in as a consistency fix) | **~86–88 (current, as of this sprint)** | See §4 |
| Target for next session | **95+** | See §5 — requires architecture-level work, not more cards |
...
**This is the explicit starting point for the next session on this thread.** Every "easy" lever — UI cards on already-visited pages, copy fixes, gate removals — has now been spent. Closing the remaining ~7–9 points requires touching session/data architecture, not adding more record-page cards. In order of leverage:

**(i) A genuinely new utility surface beyond the single-sitting decision.** Utility scores 3/10 because every session is self-contained — sharper analysis, consumed once, no reason to open Quorum *between* decisions. Candidate direction: a "revisit" surface where a past open decision can be re-asked against fresh context the user adds later, without starting an entirely new session from scratch. **This is the single highest-leverage remaining lever and also the most expensive** — it touches session architecture (how a session's lifecycle is modeled, not just its presentation), not the record page. Needs its own scoping conversation before implementation: does "re-asking" create a new session row referencing the old one, or extend the existing session's messages? How does this interact with `structural-retrieval.ts`'s existing snapshot-per-session model? Does `outcomes` need a revision history? Do not start building this without first resolving those questions explicitly with Ken.

**(ii) Outcome-to-calibration feedback made visible, not just stored, on the free tier.** `CalibrationRevealCard.tsx` already computes confidence-vs-outcome drift (`avg_delta`, pattern label, trend arrow) but is gated behind Mirror access AND a ≥3-paired-outcome threshold (`summary.dataReady`). A free, single-outcome version on the record page ("Your confidence was 7/10 going in — it worked out. That's now 1 data point toward your calibration record.") would close the Learning/Identity gap further without waiting for either gate. Lower effort than (i) — likely a new lightweight component plus a single-outcome read from the `outcomes` table, following the same "free, server-rendered, no Mirror gate" pattern already established by `BiasNoteCard` this sprint. Good candidate for a fast follow-up sprint using the same shape as RET-3.
...
**None of (i)–(iii) are a single-session sprint.** (i) and (iii) are architecture/policy decisions requiring explicit scoping with Ken before any code is written; (ii) is buildable in roughly the same shape as RET-3 (one new component, one server-side query, no schema change) and is the natural next quick win if a fast follow-up is wanted before tackling (i) or (iii).

### 6. Files touched across RET-1/2/3

**New:** `components/EmailCaptureCard.tsx`, `components/EarlyEchoCard.tsx`, `components/BiasNoteCard.tsx`, `app/api/record/[id]/echo-hint/route.ts`
...
**Unused as of this sprint (flagged for deletion, not yet removed):** `app/api/brief-access/route.ts`, `BRIEF_ACCESS_TOKEN` Railway env var — see PENDING/NEXT SESSION.

**Zero schema changes, zero new env vars across all three sprints** — `echo-hint` reuses the existing `MATCH_THRESHOLD` env var; everything else reads tables/columns that already existed (`bias_library`, `sessions_ontology`, `getStoredSessionIds()`/localStorage).
...
- **Sprints RET-1/2/3 (June 20, 2026) — pending verification / next steps:**
  ...
  - **Next session on this thread should start from SECOND-USE RETENTION STRATEGY §5 ("Path to 95+")** — the three remaining levers ((i) a revisit/utility surface, (ii) free single-outcome calibration feedback, (iii) `device_id` cold-start policy) are documented there with the explicit instruction not to start building lever (i) or (iii) without first scoping them with Ken; lever (ii) is buildable in the same shape as RET-3 if a fast follow-up is wanted.
