...
| After RET-4 (Lever (ii): free single-outcome calibration feedback) | **~89–91 (current, as of this sprint)** | See §5 — Learning/Identity gap narrowed; Utility gap (lever (i)) untouched |
| Target for next session | **95+** | See §5 — requires lever (i) and/or (iii), both architecture/policy decisions, not more cards |
...
**Update, June 21, 2026 (Sprint RET-4): lever (ii) is now shipped** — see new dated entry at the top of this document and KDD 193. Levers (i) and (iii) below are unchanged and remain the explicit starting point for the next session on this thread; neither should be started without first scoping with Ken.

Every "easy" lever — UI cards on already-visited pages, copy fixes, gate removals — has now been spent, and the one buildable-without-scoping architecture-light lever (ii) has also now shipped. Closing the remaining ~4–6 points requires touching session/data architecture or making a product-policy call, not adding more record-page cards. In order of leverage:
...
**(i) and (iii) are still architecture/policy decisions requiring explicit scoping with Ken before any code is written — neither has changed status this sprint.** (ii) is done; it was the only one of the three buildable without a prior scoping conversation.
