# QUORUM — Handover Document v21
### Date: May 2026 | Status: Sprint 19a complete (R2 threshold fix · Council context extended to all 6 initial personas)
...
Mirror modules: /api/mirror/{timeline|fingerprint|independence|rules|contradictions|outcomes|alerts|calibration|patterns}
                                                                                                            ↑ Sprint 17 new — ✅ Sprint 18b wired into UI
```

**Sprint 18b additions:**
- `components/PatternStore.tsx` — NEW. Wired into `mirror/page.tsx` `UnlockedView` between Decision Rules and Contradiction Detector. Fetches from `/api/mirror/patterns`. Three layers:
  1. **Computed headline** — client-side synthesis paragraph derived from the actual top patterns. No AI call. Detects key combinations (e.g. R4+R5 together → specific urgency-asymmetry read) and produces a plain paragraph naming what is structurally happening across the user's decisions.
  2. **"What keeps coming up"** — each rule pattern shown with (a) plain-language explanation of what the pattern actually means for the user, (b) a gold "What to do" box with one concrete actionable tip for future decisions, (c) frequency bar + plain phrase ("Almost every decision", "Recurring", etc.).
  3. **"What your decisions are made of"** — top ontology dimensions, each with (a) an italic plain description of what the dimension measures, (b) score bar, (c) plain score label ("High — a dominant factor across your decisions"). Includes a `dimSynthesis` block when all top dims are uniformly high.
- `lib/types.ts` — added `RuleType`, `RulePattern`, `DimPattern`, `PatternStoreData` types.
- `app/mirror/page.tsx` — `PatternStore` import added; "Decision Patterns" section inserted in `UnlockedView`.

**Sprint 19a additions:**
- `lib/rule-engine.ts` — R2 ambiguity threshold corrected: `ambiguity.score < 3` → `ambiguity.score < 4`. Aligns with research-confirmed definition (`identity_alignment ≥ 5 AND ambiguity ≥ 4`). Previous code fired R2 on a broader set of sessions than intended, undermining discriminant validity.
- `app/api/persona/route.ts` — `buildCouncilContext()` now injected into all 6 initial Council persona calls (first-pass, `messages.length === 0`), not just `synthesis` and `decision_brief`. Pushback calls remain excluded (user-reactive; structural re-injection would distort the pushback dynamic). No client-side changes. Log message updated: `[Persona] Council context injected for <key> (initial|synthesis) | session <id>`.

**Sprint 19a design decisions added (see KEY DESIGN DECISIONS):**
- #41: R2 ambiguity threshold permanently `≥ 4`. Do not lower.
- #42: Council context injected at first-pass for all 6 initial personas. Pushbacks excluded by design.

---

 All `plain`, `action`, `meaning`, and `return` string literals in `PatternStore.tsx` that contained apostrophes were written as single-quoted JS strings, causing Next.js build failures (`Expected ',', got 're'`). Fixed by converting all affected strings to template literals (backticks). No logic changes.

---

## DATABASE SCHEMA (current as of Sprint 18b)

No schema changes in Sprint 18b. All tables unchanged from Sprint 15a.
...
lib/
  rule-engine.ts           — ✅ Sprint 19a — R2 ambiguity threshold corrected to ≥ 4 (was ≥ 3);
                             also Sprint 16b: evaluateR1() confidence guard (< 0.55 → null, no REDIRECT)
  ...
  independence-score.ts, mirror-fingerprint.ts, ai-client.ts, supabase.ts, storage.ts
  types.ts                 — ✅ Sprint 18b — RuleType, RulePattern, DimPattern, PatternStoreData added
...
app/api/
  ...
  examiner/route.ts         — ✅ Sprint 18a — fireIndependenceScore + fireContradictions added
                              (same fire-and-forget localhost pattern as fireBiasScore; both run in
                              skipped and submit paths; Sprint 12 patch had only restored fireBiasScore)
  persona/route.ts          — ✅ Sprint 19a — buildCouncilContext() now fires for ALL 6 initial Council
                              personas (messages.length === 0, !rawMessages) in addition to synthesis
                              and decision_brief (Sprint 12). Pushbacks excluded by design.
...
components/
  PatternStore.tsx         — ✅ Sprint 18b NEW — Mirror Pattern Store UI. Fetches /api/mirror/patterns.
                             Three layers: computed headline (client-side synthesis, no AI),
                             rule patterns with plain explanation + "What to do" gold tip box,
                             dimension profile with plain meaning + score bar + dim synthesis block.
                             RULE_CONTENT map: plain + action per rule (R1–R12).
                             DIM_CONTENT map: plain meaning per ontology dimension (all 14).
                             computeHeadline(): detects R4+R5, R1+R7, R6+urgency, R2+R8 combos
                             and falls back to dominant type or top rule read.
                             computeDimSynthesis(): fires when top 3 dims all ≥ 3.5.
                             All string literals with apostrophes use template literals (build fix).
...
app/
  ...
  mirror/page.tsx          — ✅ Sprint 18b — PatternStore imported and wired into UnlockedView.
                             "Decision Patterns" section inserted between Decision Rules and
                             Contradiction Detector. Section intro: "What keeps showing up in how
                             you make decisions — not what you say about yourself, but what Quorum
...
| **18a** | **Bugfix: Independence Score + Contradiction Detector triggers restored in examiner route. — ✅ Deployed** |
| **18b** | **Mirror Pattern Store wired into Mirror page UI (PatternStore.tsx). Plain-language + actionable insight layer for all rule patterns and ontology dimensions. Computed headline synthesis (client-side, no AI). Build fix: apostrophes in template literals. — ✅ Deployed** |
| **19a** | **R2 ambiguity threshold fix (`≥ 3` → `≥ 4`). buildCouncilContext() extended to all 6 initial Council personas (was synthesis-only). — ✅ Deployed** |

---

## CURRENT STATUS (as of Sprint 18b)

### Sprint 18b — Mirror Pattern Store UI

**What shipped:**
- `components/PatternStore.tsx` (new file)
- `app/mirror/page.tsx` (PatternStore import + "Decision Patterns" section in UnlockedView)
- `lib/types.ts` (4 new types)

**How PatternStore works:**

The component fetches `/api/mirror/patterns` (auth-gated, mirror_access-gated, ≥3 session threshold) and renders three layers:

**Layer 1 — Computed headline (`computeHeadline`)**
Client-side function reads `patterns[]` and `sessions_with_rules`. Detects specific combinations:
- R4 + R5 both in top 3 → urgency-asymmetry read ("you feel urgency on decisions where the cost of being wrong is much larger than the gain of being right")
- R1 + R7 in top 3 → wrong-question-level read
- R6 + urgency flag → group-alignment-before-analysis read
- R2 + R8 → identity/values read
- Dominant FLAG type → general risk-conditions read
- Fallback → names top rule + frequency
Shown in a left-gold-bordered card above the pattern list. Fires only when ≥2 patterns and ≥5 sessions_with_rules.

**Layer 2 — Rule patterns ("What keeps coming up")**
Each `RulePattern` renders as a card with:
- Type badge: "Wrong question" (REDIRECT), "Pause first" (GATE), "Watch this" (FLAG)
- `RULE_CONTENT[rule_id].plain` — plain-language explanation of what the pattern means for the user
- `RULE_CONTENT[rule_id].action` — gold "What to do" box with one concrete actionable tip
- Frequency bar (turns gold at fire_count ≥ 3) + plain phrase ("Almost every decision — 80%", "Recurring — 30%", etc.)
- `RULE_CONTENT` covers R1–R12 (all live rules). Missing rule_id falls back to raw `pattern.description`.

**Layer 3 — Dimension profile ("What your decisions are made of")**
Each `DimPattern` renders with:
- `DIM_CONTENT[dim].meaning` — italic plain description of what the dimension measures
- Score bar (colour: gold ≥4, text-3 ≥2.5, border-hi below)
- Plain score label: "High — a dominant factor across your decisions", etc.
- `computeDimSynthesis()` fires a gold summary block when all top 3 dims ≥ 3.5 (or stakes+emotion, or uncertainty+ambiguity combos). Only shown when sessions_with_vectors ≥ 3.

**Build error fixed (apostrophes in single-quoted strings):**
Initial build failed: `Expected ',', got 're'` on line 45. Root cause: all `plain`, `action`, `meaning`, and `return` string values that contained contractions (e.g. `You're`, `don't`, `it's`) were written as single-quoted JS strings. Next.js/SWC parser treats the apostrophe as a closing quote. Fix: all affected strings converted to template literals (backticks). 48 string literals changed across `RULE_CONTENT`, `DIM_CONTENT`, `computeHeadline`, `computeDimSynthesis`, and `scoreDesc`. Zero logic changes.

**Data availability:** PatternStore reads from existing `sessions_ontology` rows. All decisions run before Sprint 18b ship immediately — no new sessions required to see data.

---

### Sprint 18a — Bugfix: Mirror triggers restored (carried forward)

**Root cause:** The Sprint 12 patch that refactored bias scoring into `/api/bias-score` only restored `fireBiasScore()` in the examiner POST. `fireIndependenceScore()` and `fireContradictions()` were never re-added.

**Fix:** Both added as fire-and-forget calls in skipped and submit paths of the examiner POST. **Files changed:** `app/api/examiner/route.ts` only.

---

#### Dynamic Persona Grid Reorder (Sprint 17 — carried forward)

After the Council finishes streaming, the grid reorders so the most ontologically relevant advisors appear first. Animated: fade out → cards snap to new positions → fade in → "Ranked by relevance to your decision" label. Fires only when order genuinely changed — no spurious animation.

**`computePersonaOrder()` rule → persona boost map (lib/personas.ts):**
...
#### Mirror Pattern Store API (`app/api/mirror/patterns/route.ts`) — Sprint 17

Auth-gated + mirror_access-gated. Aggregates rule firing frequency from `rule_engine_result` JSONB. No AI call. Threshold: ≥3 sessions. `pct` calculated against `sessions_with_rules` (not total sessions). `top_dimensions` only populated when `sessions_with_vectors ≥ 3`, capped at top 5 by avg_score.
...
37. **Contradiction Detector 7-day throttle (Sprint 18a): `RERUN_DAYS_THRESHOLD = 7` is correct and intentional. The two-pass AI pipeline is expensive. Contradictions don't change meaningfully session-to-session. The trigger fires every examiner submit, but the route decides whether to actually run. Do not remove the throttle.**
38. **PatternStore computed headline (Sprint 18b): client-side only, no AI call. `computeHeadline()` checks specific rule combinations in order of diagnostic specificity (R4+R5 first, most dangerous combination). Falls back to dominant type read, then single top rule. Only fires when ≥2 patterns AND sessions_with_rules ≥ 5 — prevents spurious reads on thin data.**
39. **PatternStore string literals (Sprint 18b): all `plain`, `action`, `meaning`, and inline `return` strings that contain apostrophes must use template literals (backticks), not single quotes. Next.js/SWC treats the apostrophe as a closing quote and fails the build. This applies to any future additions to `RULE_CONTENT` and `DIM_CONTENT`.**
40. **PatternStore data availability (Sprint 18b): component reads from existing `sessions_ontology` rows. No new sessions required — all historical decisions surface immediately on deploy.**
41. **R2 ambiguity threshold (Sprint 19a): permanently `≥ 4`. Previous code used `≥ 3`, which fired R2 on a majority of real sessions and undermined discriminant validity. Research-confirmed correct definition: `identity_alignment ≥ 5 AND ambiguity ≥ 4`. Do not lower.**
42. **buildCouncilContext() for initial personas (Sprint 19a): fires for all 6 first-pass Council calls (messages.length === 0, !rawMessages). Pushback calls are excluded by design — structural re-injection into a user-reactive exchange distorts the pushback dynamic. Synthesis and decision_brief remain wired as before.**

---

## PENDING (next sprint — Sprint 19b+)

- **R11 (Avoidance Detection)** — parked. Requires cron + `days_open` tracking. Do not implement until cron infrastructure is in place.
- **Railway cron for 30-day outcome nudges** — parked. `sessions_pending_outcomes` view exists and is correct; cron wiring deferred.

## RESOLVED / CLOSED (previously pending)

- **FLIP animation** — ✅ deployed and tested. Card shuffle fires post-examiner-submit at 1100ms. Glide confirmed, no jank on 1–2 card moves, "Ranked by relevance" label appears correctly after animation settles.
- **Share context Tier 2** — ✅ closed. Post-pushback "Share this context with all advisors" (Tier 1) covers the use case. No standalone field needed.
...
| R2c | Decision with `identity_alignment = 4, ambiguity = 4` → R2 does NOT fire (identity must be 5) | 🔲 |
...
| CC3 | Pushback exchange → no council context injected (log absent for pushback call) | 🔲 |
| CC4 | Synthesis still receives council context as before — no regression | 🔲 |
...
| Test | What to check | Expected |
|---|---|---|
| PS1 | Mirror page (unlocked) → "Decision Patterns" section appears between Decision Rules and Contradiction Detector | 🔲 |
| PS2 | User with ≥3 sessions + rule data → patterns list renders with plain descriptions + "What to do" boxes | 🔲 |
| PS3 | User with R4 + R5 both in top 3 → headline block fires the urgency-asymmetry read | 🔲 |
| PS4 | User with all top dims ≥ 3.5 → dim synthesis gold block appears in "What your decisions are made of" | 🔲 |
| PS5 | User with < 3 sessions → "X more decisions needed" nudge renders, no crash | 🔲 |
| PS6 | "Watch this" badge renders in orange-ish, "Wrong question" in blue, "Pause first" in gold | 🔲 |
| PS7 | Frequency bar turns gold for patterns with fire_count ≥ 3 | 🔲 |
| PS8 | Pattern with fire_count = 1 → "Once so far — watching for recurrence" phrase | 🔲 |
| PS9 | Dim scores: ≥ 4.0 → gold bar + "High — a dominant factor" label | 🔲 |
| PS10 | Historical sessions (pre-deploy) surface immediately — no new sessions required | 🔲 |
| PS11 | `npm run build` passes cleanly — no apostrophe syntax errors | 🔲 |
...
| Test | What to check | Expected |
|---|---|---|
| M1 | Submit examiner answers → Railway log shows `Independence score triggered for session <id>` | 🔲 |
| M2 | Submit examiner answers → Railway log shows `Contradictions triggered for session <id>` | 🔲 |
| M3 | Skip examiner → both independence + contradictions still triggered (skip path also fires) | 🔲 |
| M4 | Mirror page → Independence Score `calculatedAt` now matches today's date after a new session | 🔲 |
| M5 | Contradiction Detector last run date updates after ≥7 days have passed since previous run | 🔲 |
| M6 | Contradiction Detector with < 7 days since last run → Railway log shows `skipped_recent_run` | 🔲 |
| M7 | Anonymous session (no user_id) → independence + contradictions routes return `no_user_id`, no crash | 🔲 |
