# QUORUM — Handover Document v19
### Date: May 2026 | Status: Sprint 17 complete + Sprint 18a bugfix (Mirror triggers restored in examiner route)
...
| **18a** | **Bugfix: Independence Score + Contradiction Detector triggers restored in examiner route. Sprint 12 patch had only re-added fireBiasScore; both Mirror triggers were silently dropped. — ✅ Deployed** |

---

## CURRENT STATUS (as of Sprint 18a)

### Sprint 18a — Bugfix: Mirror triggers restored

**Root cause:** The Sprint 12 patch that refactored bias scoring into `/api/bias-score` (to fix schema column name mismatches) only restored `fireBiasScore()` in the examiner POST. `fireIndependenceScore()` and `fireContradictions()` were never re-added, so neither ran after any session since Sprint 12.

**Symptoms:**
- Decision Independence Score showed stale date (last updated 8 May) — was only recalculated when manually triggered
- Contradiction Detector showed last run 4 May — same cause; the 7-day throttle is correct and by design, but the trigger never fired to respect it

**Fix:** Added `fireIndependenceScore(sessionId, req)` and `fireContradictions(sessionId, req)` as fire-and-forget calls in both the skipped and submit paths of the examiner POST, matching the exact same localhost pattern as `fireBiasScore`. Both new functions documented in source with clear comments explaining their semantics.

**Independence Score behaviour post-fix:** Recalculates on every examiner submit/skip. Upserts one row per `session_id` — idempotent. No throttle.

**Contradiction Detector behaviour post-fix:** Triggers on every examiner submit/skip, but the route internally checks `RERUN_DAYS_THRESHOLD = 7` — if last run was within 7 days, it returns `skipped_recent_run` immediately without running the AI pipeline. This is correct — the two-pass AI contradiction pipeline is expensive and contradictions don't change meaningfully session-to-session.

**Files changed:** `app/api/examiner/route.ts` only. No schema changes.
...
37. **Contradiction Detector 7-day throttle (Sprint 18a clarification): `RERUN_DAYS_THRESHOLD = 7` in the contradictions route is correct and intentional. The two-pass AI pipeline (principle extraction + contradiction detection) is expensive. Contradictions don't change meaningfully session-to-session. The trigger fires every examiner submit, but the route decides whether to actually run. Do not remove the throttle — it is a cost control, not a bug.**

---

## PENDING (next sprint — Sprint 18b)
