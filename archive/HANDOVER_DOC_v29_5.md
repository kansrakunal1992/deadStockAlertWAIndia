# QUORUM — Handover Document v29_5
### Date: June 1, 2026 | Status: Audit R6 (Prompt Overload) analysed — deferred as TD-2, no code change. No files changed this session.
...
components/
  ...
  SynthesisCard.tsx        — ✅ Sprint D3 — synthesis POST body now includes resubmitAlertId.
...
| **Audit R6 — deferred (TD-2)** | **Prompt Overload / Context Compression: full codebase analysis of synthesis token budget and competing MANDATORY directive stacking (persona/route.ts layers 1–5, SynthesisCard.tsx examiner block). Verdict: risk is theoretical — no observed synthesis degradation, no confirmed output quality failure. Two fixes identified (examiner answers-only in SynthesisCard.tsx, compressCouncilContextForSynthesis() in persona/route.ts) but both carry small regression risk and the stage gate is pre-revenue. Decision: do not implement now. Deferred as TD-2 with trigger condition: revisit at high decision count (≥250 sessions) or on first observed synthesis quality failure attributable to directive conflict. Locked as KDD 116. No files changed. — ✅ Analysed June 1, 2026** |
...
**Last completed:** Audit R6 analysis — deferred as TD-2 (no code change), June 1, 2026
**Prior completed:** Additional Risk D — D3 (Mirror surface) — implemented June 1, 2026 (deploy pending)
...
116. **Audit R6 (Prompt Overload) is deferred as TD-2 — do not implement until ≥250-session corpus or confirmed synthesis quality failure.** The risk is that 4–5 simultaneous MANDATORY directives in the synthesis system prompt cause directive conflict or deprioritisation of earlier layers (councilContext at layer 2 vs relevanceBlock at layer 5). Analysis confirmed this is theoretical: no observed output degradation, existing 800-char persona cap and high-signal-only dim filter are material mitigations, and the ~4,500–5,000 token synthesis budget is well within model comfort. Two fixes were designed (examiner answers-only in SynthesisCard.tsx; compressCouncilContextForSynthesis() in persona/route.ts) but both carry small regression risk. The correct validation before implementation is the audit's own suggested comparison: synthesis outputs with vs without all context layers on identical decisions. Do not reopen as a build item before the TD-2 trigger conditions are met.
---
...
### TD-2 — Prompt Overload / Context Compression (Audit R6)
**Added:** June 1, 2026 (analysis session — no code change)
**Priority:** Deferred — revisit at ≥250-session corpus or on first observed synthesis quality failure attributable to directive conflict

The synthesis system prompt currently assembles 5 layers: SYNTHESIS persona prompt (~1,750 tokens), councilContext (~300–500 tokens), synthesisBlock with bias + calibration + contradiction (~250–400 tokens), conditional resubmissionBlock (~150 tokens), and relevanceBlock (~150 tokens). Four of these layers contain their own MANDATORY or NON-NEGOTIABLE directive. Total synthesis call is ~4,500–5,000 tokens on a heavily-flagged session — well within model limits, but the competing directives are a theoretical dilution risk.

**Risk verdict:** THEORETICAL — no observed synthesis degradation, no confirmed output quality failures traceable to directive stacking. Existing mitigations (800-char persona output cap in SynthesisCard.tsx, high-signal-only dim filter in buildCouncilContext) already address the token budget. Implementing now carries small regression risk with no confirmed upside.

**Two fixes identified when this is revisited:**
1. `components/SynthesisCard.tsx` — examiner context builder: strip question text, pass numbered answers only. Saves ~150–300 tokens from synthesis user message. One-line change.
2. `app/api/persona/route.ts` — add `compressCouncilContextForSynthesis()` helper: when synthesis call has synthesisBlock present, replace verbose `── COUNCIL SIGNALS ──` flag-rule question text with terse rule-ID summary line. Reduces layer-2 token footprint and removes redundancy with relevanceBlock at layer 5.

**Trigger condition:** Reopen when (a) corpus reaches ≥250 sessions and admin dashboard R7 shows synthesis council_helped delta degrading for complex sessions, OR (b) a user reports or a QA test demonstrates synthesis output that demonstrably ignores one of the MANDATORY directives. Do not implement speculatively before either condition is met. See KDD 116.
