...
lib/
  ...
  bias-scorer.ts           — ⏳ Sprint B patch (June 3, 2026): createCompletion → { provider: 'anthropic' }. Rationale: complex nested JSON; errors corrupt Ledger + Mirror fingerprint downstream.
  ...
  mirror-fingerprint.ts    — ⏳ Sprint B patch (June 3, 2026): createCompletion → { provider: 'deepseek' }. Rationale: prose portrait, user-facing narrative; DeepSeek V4 quality-competitive on prose.
  ...
  ontology-tagger.ts       — ⏳ Sprint B patch ready (June 3, 2026) — direct SDK instances removed.
                             ...
                             Apply via: ontology-tagger.PATCH.txt (2 hunks — imports block + callTagger function).
                             Sprint 27 — decision_discriminating_info rubric NOTE added (carried forward).
  structural-retrieval.ts  — ⏳ Sprint B patch (June 3, 2026): createCompletion → { provider: 'anthropic' }. Rationale: precise 2–3 sentence explanatory output consumed directly in UI.
...
app/api/
  ...
  mirror/rules/route.ts          — ⏳ Sprint B patch (June 3, 2026): createCompletion → { provider: 'anthropic' }. Rationale: hallucinated rules corrupt Mirror over time.
  ...
  persona/route.ts          — ⏳ Sprint B patch (June 3, 2026): createStream gains 3rd provider param.
  ...
  examiner/route.ts         — ⏳ Sprint B patch (June 3, 2026): two createCompletion calls updated.
  ...
  record/[id]/brief/route.ts — ⏳ Sprint B patch (June 3, 2026): briefContent createCompletion → { provider: 'deepseek' }. Rationale: template-guided narrative, prose-quality competitive.
  ...
  voice/cleanup/route.ts    — ⏳ Sprint B patch (June 3, 2026): createCompletion → { provider: 'deepseek' }. Rationale: trivial normalisation task, lowest stakes call in the stack.
                            — ✅ Sprint 23a NEW
...
**Hybrid Routing Sprint (June 3, 2026) — Sprint A DEPLOYED. Sprint B pending.**
lib/ai-client.ts rewritten with per-call provider override and ROUTING_MODE env var. Per-call provider flags written for all 15 AI calls across 9 files (Sprint B patch file ready). ontology-tagger.ts migration patch ready (removes direct SDK instances).

**Pending to complete hybrid routing:**
- Apply ontology-tagger.PATCH.txt (2 hunks)
- Apply sprint-b-ALL_PATCHES.txt across 9 files
- Set Railway env vars: ANTHROPIC_MODEL, DEEPSEEK_MODEL, ROUTING_MODE (optional — defaults to hybrid)

**Sprint R_JC (June 2, 2026) — COMPLETE, deploy pending.**
userJudgmentContext() completion sprint. Three missing longitudinal pieces shipped:
- Prior C0 principles injection: `fetchUserPrinciplesBlock()` in bias-scorer.ts. Synthesis now checks current framing against user's stated success criteria from past sessions.
- Recurring regret detection: `fetchRecurringRegretBlock()` in bias-scorer.ts. Synthesis now flags structural recurrence of bad-outcome decisions.
- Examiner bias-aware sharpening: `fetchExaminerBiasHint()` exported from bias-scorer.ts; examiner route wired. Diagnostic questions are now harder for users with confirmed blind spots.
Two files changed (lib/bias-scorer.ts, app/api/examiner/route.ts). Zero schema changes. Zero new files.

**Remaining (a) items — not yet built:**
- Decision State (post-synthesis commitment capture)
- Switch Conditions (pre-outcome what-would-change capture)
- Rule Recall at session time (surfacing prior rules during live decision)
- Monthly Judgment Review (periodic Mirror module)
