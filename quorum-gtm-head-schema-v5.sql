...
create table if not exists gtm_prospects (
  ...
  free_session_booked_at timestamptz, -- matched by email, via Google Calendar
  cta_type_used       text -- 'free_session' | 'paid_session'
);
...
alter table gtm_prospects add column if not exists free_session_booked_at timestamptz; -- matched by email, via Google Calendar
alter table gtm_prospects add column if not exists cta_type_used text; -- 'free_session' | 'paid_session' — which CTA the outreach actually used, for messaging-effectiveness learning
...
-- ── Existing-user nurture log (top pick #1) ──────────────────────────────
-- One row per user, ever — this is the idempotency guard that makes the
-- nurture email genuinely one-time. Also doubles as the attribution record
-- for anyone who converts afterward (decision_session_payments has no
-- user_id column, so this is where a nurture-driven payment gets recorded
-- instead of on a gtm_prospects row).
create table if not exists gtm_nurture_log (
  user_id          uuid primary key,
  sent_at          timestamptz not null default now(),
  session_count_at_send integer,
  cta_type_used    text, -- 'free_session' | 'paid_session'
  converted_at     timestamptz,
  paid_amount_inr  numeric
);

alter table gtm_nurture_log enable row level security;
